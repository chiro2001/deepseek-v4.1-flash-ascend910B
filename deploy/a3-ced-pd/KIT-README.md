...
本 README 描述的就是那一包的基线 `main@7497a06`（镜像内 `repo_rev` 实测同值）。
...
| 源镜像（本层从它切出） | `local/dsv41-a3-ced-pd:v3`（id `sha256:416a3e5a8234…`） |
| 工作层 | **15 层**，压缩后 **1.2 MiB** |
| 文件系统条目 | 305,791 |
| payload 文件 | 92 |
| **代码基线** | 本仓 `main` @ `7497a06`（镜像内 `/opt/dsv41/BUILD_INFO.txt` 的 `repo_rev`） |

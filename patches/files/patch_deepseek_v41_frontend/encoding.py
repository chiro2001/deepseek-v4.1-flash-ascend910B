...
# Vendored from the V4.1 checkpoint encoding/encoding.py; see LICENSE.
"""
...
def _validate_no_image_sp_tokens(msg: dict[str, Any]) -> None:
    ...
    if isinstance(reasoning_content, str) and IMAGE_PLACEHOLDER in reasoning_content:
        raise ValueError(f"reasoning_content contains image special token '{IMAGE_PLACEHOLDER}'")
...
def render_reasoning_effort(
    ...
    assert (type(effort) is int and 1 <= effort <= 100) or effort in REASONING_EFFORT_MAPPINGS, (
...
def encode_case(case: dict[str, Any], thinking_mode: str) -> tuple[str, list[dict[str, Any]]]:

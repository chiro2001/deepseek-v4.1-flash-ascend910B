...
# [DSV41 probe] Shadow eager A/B for the DSpark draft graph: after a graph
# replay, re-run the *same* draft forward eagerly on the same buffers and diff
# the produced draft token ids. Enabled with DSPARK_GRAPH_SHADOW_EAGER=1.
...
# in-graph ExternalEvent route was dropped: replay-time external-event
# record/wait/reset ordering did not match capture and left intermittent races.
_DSPARK_DEVICE_METADATA_HOST_WAIT = os.environ.get("DSPARK_GRAPH_DEVICE_METADATA", "0") == "1"
# [DSV41 fix 0005 / executor contract] How the DSpark *draft* runs its
# device-metadata kernels (DSA ``sas_metadata`` / ``dspark_swa_indices``).
#
#   sync  (default) - run them inline on the current stream, once inside graph
#                     capture and again on every eager rebuild. Nothing is
#                     submitted to a DeviceMetadataExecutor, no ExternalEvent is
#                     armed, no in-graph wait is recorded, and the draft never
#                     touches ``runner.device_metadata_executor``.
#   async           - the old submit/ExternalEvent contract. NOT IMPLEMENTED, see
#                     DSPARK_DRAFT_METADATA_ASYNC_UNIMPLEMENTED: it raises instead
#                     of half-running.
_DSPARK_DRAFT_METADATA_MODE = os.environ.get("DSPARK_DRAFT_METADATA_MODE", "sync").strip().lower()
if _DSPARK_DRAFT_METADATA_MODE not in ("sync", "async"):
    raise ValueError(
...
DSPARK_DRAFT_METADATA_ASYNC_UNIMPLEMENTED = (
    ...
    "frontier set is identical to the capture submission of that descriptor, and (3) a matching "
    "release() after every draft forward. The pre-0005 code was missing (2)/(3) or submitted into "
    "the main model's shared executor, which either deadlocks the graph on an ExternalEvent that "
    "the per-step path never re-arms ('The previous device metadata submission has not been "
    "released' / 'Device metadata frontiers changed for an existing full-graph batch descriptor'). "
    "Run with DSPARK_DRAFT_METADATA_MODE=sync (default)."
)
...
class AscendSpecDecodeBaseProposer(SpecDecodeBaseProposer):
    ...
    def __init__(self, vllm_config: VllmConfig, device: torch.device, pass_hidden_states_to_model: bool, runner=None):
        ...
        # [TARGETPOS-FIX] Resident staging buffer for the target runner's per-step
        # ``target_positions``.  The target runner hands that tensor out freshly
        # allocated on every step (``self.positions[token_indices]`` - advanced
        # indexing in ``gpu_model_runner._get_positions`` / the Ascend runner's
        # ``propose_draft_token_ids`` override), so its address moves on every
        # replay while an ACLGraph replay never re-runs Python.  Refresh one
        # persistent buffer in place once per step and hand out only a slice of
        # it, exactly like ``dsa_v1``'s resident ``start_pos_draft``.  Created
        # lazily so that dtype/device are taken from the tensor the target runner
        # actually passes (it is int64, while this drafter's own ``self.positions``
        # above is int32).
        self._draft_target_positions: torch.Tensor | None = None
    ...
    def _stage_target_positions(self, target_positions: torch.Tensor) -> torch.Tensor:
        """[TARGETPOS-FIX] Return ``target_positions`` as a slice of a resident buffer.

        Address invariance, not values, is the point: the draft ACL graph is
        replayed by ``ACLGraphWrapper`` (``entry.aclgraph.replay()``) without
        re-running this Python, so every tensor it is *told* about must live at an
        address that does not move between steps.  ``target_positions`` arrives
        from the target runner as a fresh allocation per step
        (``_get_positions(token_indices)`` -> advanced indexing ->
        ``self.positions[token_indices]``), which is the drift the ``[argprobe]``
        hot probe reports as ``drift={'target_positions': 1}``.

        The buffer is created once, aligned to the dtype/device of the incoming
        tensor (the runner's positions are int64, this proposer's own
        ``self.positions`` is int32 - a hard-coded dtype would silently cast), and
        every later call must agree with that dtype/device: silently re-allocating
        on a dtype change would itself move the address.  ``n`` above the buffer
        capacity is a hard error rather than a silent truncation.
        """
        if target_positions.dim() != 1:
            # M-RoPE / xdrope variants hand out ``(dim, num_tokens)`` slices of a
            # persistent buffer already, so their address is invariant as-is.
            return target_positions
        num_tokens = target_positions.shape[0]
        buffer = self._draft_target_positions
        if buffer is None:
            buffer = torch.zeros(
                self.max_num_tokens,
            ...
            self._draft_target_positions = buffer
        if buffer.dtype != target_positions.dtype or buffer.device != target_positions.device:
            raise RuntimeError(
                "[TARGETPOS-FIX] target_positions dtype/device changed mid-run "
                f"({buffer.dtype}/{buffer.device} -> {target_positions.dtype}/{target_positions.device}); "
                "refusing to swap the resident buffer out from under the captured draft graph"
            )
        if num_tokens > buffer.shape[0]:
            raise ValueError(
                "[TARGETPOS-FIX] target_positions has "
                f"{num_tokens} elements but the resident buffer holds {buffer.shape[0]} "
                "(capacity = self.max_num_tokens); refusing to truncate"
            )
        buffer[:num_tokens].copy_(target_positions)
    ...
    def _propose(
        ...
        # [TARGETPOS-FIX] Rebind the per-step tensor to the resident buffer before
        # its first consumer (``set_inputs_first_pass``, which feeds the DSpark /
        # DFlash input-expansion kernel through ``target_positions_ptr``) and keep
        # the token count for the graph input rebound further below.
        num_target_positions = target_positions.shape[-1]
        target_positions = self._stage_target_positions(target_positions)
        ...
        multi_steps_attn_metadata, attn_metadata_i = self.build_draft_attn_metadata(
            common_attn_metadata, num_input_tokens, num_tokens, batch_descriptor=batch_descriptor
        )
        ...
        active_device_metadata_executor = (
            self._dspark_device_metadata_executor() if self.method == "dspark" else None
        )
        ...
        with set_ascend_forward_context(
            ...
            model_inputs: dict[str, Any] = {
                ...
                # [TARGETPOS-FIX] The draft ACL graph binds this input while it is
                # captured, and both capture paths (``dummy_run`` here and
                # ``AscendDSparkProposer.dummy_run``) bind
                # ``self._get_positions(...)`` - a slice of this drafter's own
                # persistent positions buffer.  Replay does not re-run Python, so
                # hand out that same resident slice again: the declared input
                # address is then invariant across every replay instead of being a
                # fresh ``self.positions[token_indices]`` allocation from the
                # target runner.  The captured callable never dereferences this
    ...
    def _probe_shadow_eager_dspark(
        self,
        draft_token_ids: torch.Tensor,
        model_inputs: dict[str, Any],
    ) -> torch.Tensor:
        """Probe-only: compare the ACL-graph draft output with an eager rerun.

        The eager rerun reuses the identical input buffers and the identical
        attention metadata object, so any divergence is caused by what the
        capture baked into the graph, not by different inputs. The graph output
        is cloned first because both paths write into ``self._dspark_draft_buffer``.
        """
        global _DSPARK_SHADOW_DONE
        if self.method != "dspark" or not self.use_cuda_graph:
            return draft_token_ids
        if _DSPARK_SHADOW_DONE >= _DSPARK_SHADOW_STEPS:
            return draft_token_ids
        if getattr(self, "_enable_probabilistic_draft_probs", False):
            logger.warning("[dspark-graph-probe] probabilistic draft sampling; skipping shadow eager")
            _DSPARK_SHADOW_DONE = _DSPARK_SHADOW_STEPS
            return draft_token_ids

        _DSPARK_SHADOW_DONE += 1
        graph_ids = draft_token_ids.clone()
        forward_context = get_forward_context()
        prev_mode = forward_context.cudagraph_runtime_mode
        prev_capturing = getattr(forward_context, "capturing", False)
        try:
            forward_context.cudagraph_runtime_mode = CUDAGraphMode.NONE
            forward_context.capturing = False
            eager_ids = self._run_merged_draft(**model_inputs)
        except Exception as exc:  # pragma: no cover - probe only
            logger.warning("[dspark-graph-probe] shadow eager rerun failed: %r", exc)
            return graph_ids
        finally:
            forward_context.cudagraph_runtime_mode = prev_mode
            forward_context.capturing = prev_capturing

        if not isinstance(eager_ids, torch.Tensor):
            return graph_ids
        eager_ids = eager_ids.clone()
        same = int((graph_ids == eager_ids).sum().item())
        total = int(graph_ids.numel())
        logger.warning(
            "[dspark-graph-probe] shadow eager compare #%d: graph==eager=%d/%d (%.1f%%) | "
            "graph[0]=%s eager[0]=%s | graph[1]=%s eager[1]=%s",
            _DSPARK_SHADOW_DONE,
            same,
            total,
            100.0 * same / max(total, 1),
            graph_ids[0].tolist() if graph_ids.dim() > 1 else graph_ids[:8].tolist(),
            eager_ids[0].tolist() if eager_ids.dim() > 1 else eager_ids[:8].tolist(),
            graph_ids[1].tolist() if graph_ids.dim() > 1 and graph_ids.shape[0] > 1 else None,
            eager_ids[1].tolist() if eager_ids.dim() > 1 and eager_ids.shape[0] > 1 else None,
        )
        return graph_ids
    ...
    def _dspark_device_metadata_executor(self) -> DeviceMetadataExecutor | None:
        """Executor-shaped *gate* for the DSpark draft metadata contract.

        The per-step path (:meth:`build_draft_attn_metadata`) and the capture path
        (``AscendDSparkProposer.dummy_run``) share one contract, selected by
        ``DSPARK_DRAFT_METADATA_MODE``:

        * ``sync`` (default): the draft metadata kernels run synchronously on the
          current stream - no in-graph ExternalEvent waits, no dedicated metadata
          stream and *no submission at all*. A dedicated executor instance is
          still returned because it is the gate that makes the DSA builder defer
          its metadata into tasks (``builder is DeviceMetadataTaskProvider``);
          nothing is ever submitted to it, so it never arms an event and nothing
          has to be released.
        * ``async``: raises - see ``DSPARK_DRAFT_METADATA_ASYNC_UNIMPLEMENTED``.

        ``runner.device_metadata_executor`` is *never* returned. That instance is
        the target model's: one ``_submission_in_flight`` flag plus a frontier
        tuple per ``batch_descriptor``, both owned by the per-step target
        submission. Submitting draft tasks to it either trips "The previous
        device metadata submission has not been released" or rewrites the target's
        frontier set ("Device metadata frontiers changed for an existing
        full-graph batch descriptor").
        """
        if self.method != "dspark":
            return None
        if not _DSPARK_DRAFT_METADATA_SYNC:
            raise RuntimeError(DSPARK_DRAFT_METADATA_ASYNC_UNIMPLEMENTED)
        executor = getattr(self, "_draft_device_metadata_executor", None)
        if executor is None:
            executor = DeviceMetadataExecutor()
            self._draft_device_metadata_executor = executor
        return executor
    ...
    def build_draft_attn_metadata(
        ...
        device_metadata_executor = (
            self._dspark_device_metadata_executor()
            if (
                self.method == "dspark"
                and self.dcp_size == 1
        ...
        if device_metadata_executor is not None and device_metadata_tasks:
            if _DSPARK_DRAFT_METADATA_SYNC:
                # Sync contract [fix 0005]: run the metadata kernels on the current
                # stream. Graph capture/replay and the eager shadow rerun are then
                # stream-ordered after the metadata writes with no event or stream
                # juggling at all. Costs one metadata kernel on the critical path
                # but removes every async ordering variable.
                for _task in device_metadata_tasks:
                    _task.run()
            else:
                # [fix 0005] ``async`` is refused by
                # ``_dspark_device_metadata_executor`` before this branch can run;
                # the two statements below are the tripwire that keeps a future
                # async re-introduction from silently touching the main model's
                # executor (one in-flight submission + one frontier set per
                # descriptor, owned by the target model).
                if device_metadata_executor is getattr(self.runner, "device_metadata_executor", None):
                    raise RuntimeError(
                        "[DRAFT-EXEC-CONTRACT] refusing to submit draft device-metadata tasks to the "
                        "main model's DeviceMetadataExecutor: that instance owns the target model's "
                        "single in-flight submission and its per-batch_descriptor frontier set."
                    )
                raise RuntimeError(DSPARK_DRAFT_METADATA_ASYNC_UNIMPLEMENTED)

# [MOE-NONFINITE] probe_moe_mask/token_dispatcher.py 的超集：
#   * 保留 MOE-MASK-RANGE（V41_MOE_MASK_RANGE=1）
#   * 新增 V41_MOE_ZERO_NONFINITE=1：只把「无效行里 **非有限** 的元素」清零
#
# 动机（正确性线 + 未初始化输出审计的合成结论）：
#   token_combine 用 expanded_row_idx（无效行为 -1）索引读回 permuted 行，再乘 topk_weights（无效行为 0）。
#   0 × 任何有限值 = 0，所以有限垃圾**不可能**污染输出；
#   但 0 × Inf/NaN = NaN ⇒ 只有非有限值会真的传播。
#   因此「只零化非有限元素」在语义上应当是 **零影响的**（no-op unless pollution exists），
#   同时又能切断唯一的传播通道。与 V41_MOE_ZERO_INVALID（全部清零，会改变吸引子）不同。
#
# 热切换：V41_MOE_ZERO_NONFINITE_FILE=/tmp/v41_moe_nf（内容 "1"/"0"），0.25 s 轮询，
#         用于同会话交错 A/B。
...
# [MOE-NONFINITE] 只零化「无效行里的非有限元素」（见文件头注释）。默认关。
_MOE_NF = _os_mask_range.environ.get("V41_MOE_ZERO_NONFINITE", "0") == "1"
_MOE_NF_FILE = _os_mask_range.environ.get("V41_MOE_ZERO_NONFINITE_FILE", "")
_MNF = {"v": _MOE_NF, "last": 0.0, "mtime": -1.0}
...
def _moe_nf() -> bool:
    ...
    import time as _t
    now = _t.monotonic()
    if now - _MNF["last"] < 0.25:
        return _MNF["v"]
    _MNF["last"] = now
    try:
        st = _os_mask_range.stat(_MOE_NF_FILE)
    except OSError:
        return _MNF["v"]
    if st.st_mtime == _MNF["mtime"]:
        return _MNF["v"]
    _MNF["mtime"] = st.st_mtime
    try:
        with open(_MOE_NF_FILE) as _f:
            _MNF["v"] = _f.read().strip() == "1"
    except OSError:
        pass
    return _MNF["v"]
...
class TokenDispatcherWithAllGather(MoETokenDispatcher[MoEAllGatherCombineMetadata]):
    ...
    def token_dispatch(
        ...
        # ==== [MOE-NONFINITE] 只零化无效行的非有限元素（默认关；文件可热切换）====
        # 依据：unpermute 用 expanded_row_idx == -1 索引读回时，0（权重）× Inf/NaN = NaN，
        # 这是垃圾行污染有效输出的**唯一**通道（有限垃圾被 0 权重压掉）。
        if _moe_nf():
            try:
                _inv = expanded_row_idx < 0
                _x = sorted_hidden_states
                _badrow = (~torch.isfinite(_x)).any(dim=-1) & _inv
                _x.masked_fill_(_badrow.unsqueeze(-1), 0)
                if dynamic_scale is not None:
                    _dsbad = (~torch.isfinite(dynamic_scale)) & _inv
                    dynamic_scale.masked_fill_(_dsbad, 0)
            except Exception:
                pass

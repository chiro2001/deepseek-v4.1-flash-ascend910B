...
|---|---|---|---|---|---|
| 1 | `$A/models/deepseek_v41/engram_hbm.py` | `files/engram_hbm.py` | `6f227a749aa6ba6f1290446611202028` | （总是开）`V41_ENGRAM_HOST_RESIDENT=1` `V41_ENGRAM_LOCAL_OWNER_FILE=/tmp/v41_engram_localowner` | Engram host 常驻 + `LOCAL_OWNER=fast`（走 numpy/numba plan 分支）。route 少一次 metadata all_gather 与 ids all_to_all |
...
| 6 | `$A/models/deepseek_v41/model.py` | `files/model.py` | `5b7c45261e2d63838b9e4a25f87ad350` | （与 #1 配对） | Engram host-resident 路径的模型侧改动（probe 版整文件） |
...
| 12 | `patches/admission_gate.patch`（git apply 到 `$VLLM_ROOT=/vllm-workspace/vllm`） | `admission_gate.patch` | `8243dff6c9dc3d87805f1dfd7820c23f` | `VLLM_ADMISSION_GATE=1` | 预填充隔离，prefill 不饿死 decode |

> 注：#3/#4 是**新增文件**（不是覆盖），构建脚本会直接 COPY；其余是覆盖 + `.a2orig` 备份。

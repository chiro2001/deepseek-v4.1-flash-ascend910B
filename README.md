...
**怎么读这两张表**（以默认的 A 表为例）：总吞吐在 64 并发时达到 **719.5 tok/s**（单流的 8.26×），
但单流已被压到 20.2 tok/s（效率 22.3%）。交互式场景（要低延迟）**并发 2–4 是甜点区**
（单流仍有 80–89 tok/s）；离线批量（要总吞吐）拉到 32–64。
**`DRAFT_GRAPH` 怎么选见 C 表 —— 低并发开、高并发关。**

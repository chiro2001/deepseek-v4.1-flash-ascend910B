...
> **v8 新增 §v8CONC**（最下面）：**并发吞吐重采**（1/2/4/8/16/32/64 × 2 rep，全部 64/64），
> 即 README §3.2 那张表的数据源，也是"护栏撤销"的回归证据。
...
# §v8CONC —— 并发吞吐重采（2026-09-20，v8 发布默认配置）

> **口径**（与 README §3.2 完全一致）：
> prompt **每条精确 1024 token**（服务端 `/tokenize` 校准，64/64 命中、最大偏差 0）、
> 输出 **256 token**（`ignore_eos` 强制生成满）、语料 `data/dihuo.txt`（刘慈欣《地火》，
> 需自行下载，见 `tools/fetch_corpus.sh`）、**每请求不同正文切片 + 轮换 5 个问题**
> ⇒ 任意两条内容不重叠、无 prefix cache 复用。
> 每档跑完同一批 **64 条**请求、**2 rep 取中位数**。
>
> **配置** = 本包默认：`MAX_SEQS=64 PREFIX=1 GPU_UTIL=0.92 STATIC_KERNEL=1`
> + `ENGRAM_DEVICE_INDEX=auto`（A3 探测通过）+ `DRAFT_GRAPH=0` + `ASYNC=0`；
> **无 PGO 产物**（A3 镜像的 libpython md5 `eaea156e…` 与 A2 的 `f1ebbee1…` 不同 ⇒
> 按设计自动降级 `PYTHON_PGO=0`，见 `scripts/serve_a3.sh` 注释）。

| 并发 | 单流 tok/s | 总吞吐 tok/s | 加速比 | 单流效率 | A | TTFT |
|---:|---:|---:|---:|---:|---:|---:|
...
1. **7 档 × 2 rep 全部 `ok=64/64`，服务全程存活。**
   这不只是吞吐数据 —— 同一个扫描在**撤销 `model_runner_v1.py` 的 DMQ 护栏之前是 57/64**
   （`ScatterElements 0x91` → `ERR00100` → HCCL watchdog）。
   ⇒ **"64/64" 本身就是那条撤销决定的验收判据**，见 `CHANGELOG.md` §4.3(a)。
2. **单流与总吞吐要分开看**。总吞吐在 64 并发到 **719.5 tok/s**（单流的 8.26×），
   但单流被压到 20.2 tok/s。交互式（低延迟）**并发 2–4 是甜点区**（单流仍有 80–89 tok/s）。
3. **A 全程 2.78–2.88**（正常范围）。若某次复测 A 明显偏高（>3.5）且文本在复读，
   吞吐是虚高的，不能当交付数据（判据见 §2/§3）。

## 与上一版表格的差异（诚实记录）

| 项 | v5 表格（2026-09-17） | **v8（本表）** | 变化 |
|---|---:|---:|---|
| 并发 1 单流 | 96.3 | 90.3 | **−6.2%** |
| 并发 64 总吞吐 | 595.8 | **719.5** | **+20.8%** |
| 并发 1 TTFT | 0.27 s | **0.20 s** | −26% |

两版**方法与工具完全相同**，差异**未逐项归因**：候选因素包括 KV 容量变化
（`GPU_UTIL` 0.94→0.92：3,088,412 → 2,823,080 tokens）、Engram device-index 入图
（单卡 A/B 实测并发 1 快 1.1 ms、并发 4 快 3.2 ms，见 `CHANGELOG.md` §0）、
以及**同机共租负载**（该机 0-3 号卡长期有外部任务，宿主 CPU 会被抢）。
**不要把这里的差值当成某个改动的归因结论** —— 要归因必须做同会话 A/B。
...
原始数据随包：`results/bench/conc_dihuo_v8.json`（对比用的 v5 版本是 `conc_dihuo.json`）。
**A2 上跑同一命令即可得到同口径对比** —— 注意 A2 的 8 张卡与宿主 CPU 都更弱，
预期总吞吐明显更低、单流也更低；**但 A（接受长度）应当与 A3 同量级**（A 与平台无关，
是"软件装对没装对"的锚点）。

...
* **"关多流"在 D 上到底关掉了什么 —— 已补齐（2026-09-25 22:18–22:58）**：
  只重启 D、只改这两个开关，`ms=0` 是 **1 条计算流**，`ms=1` 变成 **3 条计算流**
  （135 主 / 133 注意力 / 132 MoE–共享专家）+ 独立 allreduce 流 + 一条 AICPU 流，
  流数 10 → 12。同一批 4×144K：`ms=0` **4/4 PASS**、`ms=1` **0/4 全乱码**（复现两次）。
  完整的流清单、步数对齐后的数字、以及"缺哪条同步仍未证实"的边界，
  见 [`../evidence/ced_ms_switch_20260925/README.md`](../evidence/ced_ms_switch_20260925/README.md)。

  ⚠️ 一个容易踩的坑已记录在那份文档 §2：第一版对比没有对齐 decode 步数
  （`ms=0` 7 个 token 就停、`ms=1` 打满 64 个），会把"decode 更长"误读成
  "开关导致工作量变化"。补采 `max_tokens=8` 后再比才干净。

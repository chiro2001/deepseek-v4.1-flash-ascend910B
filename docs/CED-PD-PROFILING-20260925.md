...
结论：**能力已验证（能，而且已经跑出结果）；但"同一角色、只改开关"的对照数据
还没有** —— 现有 4 份 A3 捕获里，P 侧两份都是 `MULTISTREAM=1 DSA_OVERLAP=1`，
D 侧两份都是 `MULTISTREAM=0 DSA_OVERLAP=0`，没有同角色反差。
...
| `ced_prof_p_0925_171906/prof` | P（CED 20 层） | **1** | **1** | 4.3 G | ⬜ 未分析 |
| `ced_base_d_ms0_0925_184318/prof` | D（40 层） | **0** | **0** | 811 M | ✅ |
| `ced_prof_d_0925_165058/prof` | D（CED，带 128-token 重放） | **0** | **0** | 1.2 G | ⬜ 未分析 |
...
1. 补 **同角色反差**：给 D 采一份 `MULTISTREAM=1 DSA_OVERLAP=1`（乱码配置）
   的捕获，与现有 `=0/0` 的 D 捕获做流级 diff。这是回答"关多流到底关掉了
   什么、为什么它能修乱码"的最短路径。由于该配置会输出乱码，用**短请求**
   采集即可（只要流的行为，不需要答案对）。

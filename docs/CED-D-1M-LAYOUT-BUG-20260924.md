...
逐 Q 行复算 kernel 会对哪些列发起访存，共**三个臂**：

- `legacy`：整行块表 + 完整 `seqused_ori`（修复前的行为）
- `clip`：rebase 后的窄表 + 相对 `seqused_ori`（修复）
- `final`：**末 token 单步**（修复**不**覆盖它，所以必须天然安全）

一条命令跑全部判据：

```bash
python3 tools/ced_swa_clip_verify.py --lint-code --sweep-max 2999
```

```text
   prompt       held  legacy OOB  clip OOB  final OOB    clip 列宽   clip 越列
  1019847 [7966, 7967]           1         0          0          2         0
  1048576 [8190, 8191]           0         0          0          2         0
   900000 [7030, 7031]           1         0          0          2         0
   144404 [1127, 1128]           1         0          0          2         0
     8193       [63]             1         0          0          1         0
      999     [6, 7]             1         0          0          2         0
      256     [0, 1]             0         0          0          2         0
      255     [0, 1]             0         0          0          2         0
...
== 全扫 N=2..2999 ==
  clip 越界 0 例（需 0）
  末 token 步越界 0 例（需 0）
  legacy 越界 2722 例（应 >0，说明修复有意义）
  触发规则核验：legacy 应为 N≥256 且 N%128≠0 ⇒ 预期 2722 例
```
...
的实测一致。全扫 `N=2..2999` 里 legacy 越界 **2722** 例，与规则
`N≥256 且 N%128≠0` 的预测数**精确相等**（0 处不符）。

**末 token 单步为什么天然安全：** 该步 `S1=1`、`s1StartIdx=s1EndIdx=0`、
`Lori=N`，于是 `oriMaskRight = N-1`、`oriMaskLeft = max(N-128, 0)`，窗口正好是
`[N-128, N-1]`，需要的列只有末尾 1~2 页；而这一页此刻**必然已属于该请求**
（末 token 的位置 `N-1` 要写进 `(N-1)//128` 这一页，`slot_mapping` 需要它），
SWA 回收又只丢弃窗口之外的块。三臂扫描里 `final` 在 `N=2..2999` 上**零越界**，
所以修复不需要覆盖该步。

### 修复相对「完整 128 窗口」的偏差有多大（已量化）

裁剪后 replay 块内 query `i` 的可见长度是
`min(128, (S + i) - S_page*128 + 1)`，也就是**只有 replay 块最前面
`(128 - S%128) - 1` 个 query 的窗口会短于 128**，其余恢复满窗
（`tools/ced_swa_clip_verify.py` 的 `window_profile`）：

| prompt N | `S % 128` | 短窗 query 数 | 最短可见 | 首次满窗 |
|---:|---:|---:|---:|---:|
| 1,019,847（真实 1M 例） | 70 | 57 / 128 | 71 | 第 57 个 |
| 1,048,576（对齐） | 127 | 0 / 128 | 128 | 第 0 个 |
| 144,404 | 19 | 108 / 128 | 20 | 第 108 个 |
| 900,000 | 31 | 96 / 128 | 32 | 第 96 个 |

**这是相对论文「重放最近 nwin 个 token」的近似，不是等价实现**，需要明确两点：

1. **它严格优于修复前的行为。** 修复前这些 query 会去读**物理块 0**（null block 的
   那一列），即把 57 个零值 KV 以正常权重混进 softmax；修复后它们只看到本请求
   真实持有、且由 replay 本批写出的 KV。换句话说，修复把"绕回后才爆"的情形
   拉回到"未绕回时"的行为上——而未绕回正是历史上 144K/短针全过的状态。
2. **更低损的替代是方案 B（保留 3 页）。** 对**下半层**（层 0–19，G2–G6），
   P 在 prefill 时确实算过 `[S-127, S-1]` 这段，只是被两端 manager 回收了；
   保留 `W-1+R = 255` 个 token（`cdiv(255,128)+1 = 3` 页）就能让这些 query 拿到
   满窗。**但上半层**（层 20–39，G7–G11）P 根本没算这段（CED 的 P 在层 20 截断），
   D 侧那段历史**不存在**，所以上半层无论如何都必须靠裁剪——方案 B 只能补下半层。
   方案 B 需要跨 core 改两端的块保留上限（P 侧那页此刻已被释放），改动面更大，
   因此先用方案 A 把正确性钉住，方案 B 作为保真度改进项。

> 待上机验证的一条：`N=144404`（历史上"通过"的那种长度）在修复后是否仍然通过。
> 若通过，说明裁剪没有破坏原本工作的情形；这是修复的最低回归要求。

...
| `stream` @ `MAX_SEQS=32`（2.82M KV）＋ 8 路并发 | 11 次请求 / 222 s | 内容全对；★ 工具臂**格式**变化（见下） |

★★ **一处"判据绑错对象"的更正**（真机数据逼出来的，与 `126`/`127` 同族）：
`stream` 的 520k 工具臂一开始被我自己判成 **FAIL**，理由写的是"没走 `tool_calls`"。
但把流式全文打出来看，模型输出的是**逐字正确的校验码**
（`ZQ7K-3341-VX2M-8890-HT4P-5527-RB9N-6014-PLM3-7712-CDF8-2205`），只是**用文字回答**而不是调工具；
而**同一 prompt 的短上下文对照**会走 `tool_calls`。
⇒ 这是**格式/指令遵循**的差异（**记录项**），**不是内容损坏**（**bug**）。
⇒ 判据已改成两层：**主判据 = 校验码是否逐字出现**（工具参数**或**文字回答里都算）；
**格式（tool_calls/text/neither）只记录、不参与通过与否**。
（教训：**"没按我预期的方式做" ≠ "做错了"**；判据要绑在**内容**上，不能绑在**形式**上。）

...
## 7. 当前状态（2026-09-26 更新）

* §1 的冒烟：**已完成**，基础设施可用、命中路径正确（§4）。
* CED 臂实测：**144K 可用，1M 崩**（§6）。崩溃点是连接器的 12-group 形状假设。
* **下一步明确**：改 `mooncake_hybrid_connector.py::start_load_kv` 的 group 形状校验，
  让它接受"命中时只有部分 group 有公共前缀块"的形态；然后再看 1M 是否还有第二个问题
  （命中不加速）。
* 验收表里"缓存命中"一行：基线口径 **已验证**；CED 口径 **144K 可用 / 1M 阻断**。

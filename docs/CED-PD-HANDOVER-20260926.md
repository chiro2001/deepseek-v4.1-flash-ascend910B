...
### 5.2 缓存命中：**2026-09-27 已转为交付口径**

`PREFIX` 默认 **1**（关掉用 `PREFIX=0`）。转正依据见
[`CED-PD-CACHE-HIT-PLAN-20260925.md`](CED-PD-CACHE-HIT-PLAN-20260925.md) §11–§13：
144K/1M 常规·整池·交错命中全部正确、冷热逐字节一致（≈16–18×），
且三处会打死引擎的问题已修并有真机触发痕迹。
DSpark 同样已转正（D 侧默认 `SPEC=1 DRAFT_GRAPH=1`）。

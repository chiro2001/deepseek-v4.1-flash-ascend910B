...
| 1M | **101.22 s** | **280.34 s** | **2.77×** | 4/4 PASS | 4/4 PASS※ |
...
| 1M | 101.217 s | 280.34 s | **2.77×** | 是（CED 4/4、基线 4/4） |
...
CED 臂的完整基准已在 §4.3 用同一工具实测（下表是当时用 TTFT 折算的估计值，
保留在这里是为了对照两者是否一致 —— 结论一致）：

| 上下文 | CED-PD（P=20 层） | 基线（P=40 层） | 比值 |
|---|---:|---:|---:|
| 144K | 143,989 / 10.952 s = **13,147 tok/s** | 6,599 | **1.99×** |
| 1M | 999,396 / 101.217 s = **9,874 tok/s** | 3,557 | **2.77×** |

144K 的 1.99× 与层数比（40/20）完全吻合；1M 的 2.77× 更高，说明基线在长序列下的
退化比 CED 更严重（或者说 CED 省掉的正是最贵的那部分）。下表补齐后可以确认。


### 4.3 两臂**同口径**的全量对照（2026-09-25 23:38–23:48 补齐）

同一天、同一台机、同一批请求，两臂各跑一遍 `ced_pd_bench.py`
（`ignore_eos`、128 token、两次重复）与 `ced_pd_acceptance.py --mode multiturn`。

#### prefill 与 decode

| 上下文 | CED prefill tok/s | 基线 prefill tok/s | **prefill 比** | CED decode ms/step | 基线 decode ms/step |
|---|---:|---:|---:|---:|---:|
| 32K | 12,536 / 12,584 | 7,219 / 7,223 | **1.74×** | 25.3 / 25.3 | 25.4 / 25.5 |
| 144K | 13,676 / 13,645 | 6,599 / 6,593 | **2.07×** | 26.8 / 27.0 | 26.8 / 26.7 |
| 1M | 9,897 / 9,903 | 3,557 / 3,558 | **2.78×** | 37.9 / 37.9 | 37.4 / 37.6 |

**decode 步时两臂几乎完全一样**（25.3 vs 25.4、26.8 vs 26.8、37.9 vs 37.5，差 <1.1%）。
这是符合预期的：CED 只改了 **P 的层数**，D 在两臂里都是"全 40 层 + 同一份 KV"，
decode 计算量不变。⇒ **CED 的全部收益在 prefill 侧**，这也说明我们确实只改了该改的地方。

prefill 比随上下文放大：**1.74× → 2.07× → 2.78×**。
32K 时还不到层数比 2.0×（短序列下 P 有别的固定开销），
1M 时达到 2.78×（长序列下省掉的正是最贵的那 20 层）。

#### 多轮（修正口径后的真实长度）

| 上下文 | CED 三轮 `prompt_tokens` | CED 每轮 wall | CED 判决 | 基线每轮 wall | 基线判决 | 比 |
|---|---|---|---:|---:|---:|---:|
| 144K | 144,105 / 144,131 / 144,157 | 10.80 / 10.85 / 10.84 s | **3/3 PASS** | 21.93 s | 3/3 PASS | **2.03×** |
| 1M | **999,510 / 999,536 / 999,562** | 101.32 / 101.20 / 101.26 s | **3/3 PASS** | 280.5 s | 3/3 PASS | **2.77×** |

⇒ **CED 臂的 144K/1M 多轮已按真实长度重跑通过**，此前那两条 18K / 125K 的
缩水记录（见 [`../evidence/ced_acceptance_20260925/README.md`](../evidence/ced_acceptance_20260925/README.md) §2）
已被替换。两臂的 `prompt_tokens` 完全一致，所以是干净的同口径对照。

#### 一句话总结

| | P（prefill） | D（decode） |
|---|---|---|
| CED 相对基线的变化 | **1.74× / 2.07× / 2.78×**（32K/144K/1M） | **无变化**（<1.1%） |
| 正确性（144K/1M 四针、流式、多轮） | 全部 PASS | 全部 PASS |


### 4.5 吞吐（并发扫描，2026-09-25 23:54–23:56）

工具 `tools/bench_concurrency.py`；2K prompt / 128 输出 token、`ignore_eos`、
4 个请求用**互不重叠**的语料切片（步长 206,282 字符、4×步长 = 语料全长）
且长度精确校准到 2048 tok（4/4 命中，偏差 0）⇒ 没有前缀复用成分。
`MAX_SEQS=4`，所以并发只扫到 4；再高会被调度器排队、数字失真。

| 并发 | 单流 decode tok/s | **总吞吐 tok/s** | 加速比 | 单流效率 | TTFT |
|---:|---:|---:|---:|---:|---:|
| 1 | 41.1 | **41.4** | 1.00× | 100% | 0.48 s |
| 2 | 37.2 | **70.6** | **1.70×** | 90.7% | 0.66 s |
| 4 | 32.3 | **114.5** | **2.77×** | 78.7% | 0.99 s |

读法：

* **总吞吐随并发接近线性**（4 并发 2.77×），说明 D 侧的 TP8 在批量解码时利用率还在上升；
* **单流效率递减**（100% → 90.7% → 78.7%）是典型的批处理摊薄；
* **TTFT 从 0.48 s 涨到 0.99 s**（并发 4）——2K prompt 下 P 只要 0.16 s 左右，
  涨的主要是排队 + 批内 prefill 一起做的时间；
* `接受长度 = 0.00` 是预期的：CED 口径 `SPEC=0`（draft 关），所以没有投机解码的收益
  ——所以这里的吞吐是**纯自回归**的诚实数字，不能与开了 DSpark 的基线数字直接比。

⚠️ 本表只有 **CED 臂**。基线臂的同口径吞吐未测（要再切一次服务）。
由于 CED 只改 P、decode 步时两臂实测几乎相同（§4.3），
**预估基线总吞吐与本表接近**，但那是推断，不是实测。

...
| 6 | 缓存命中 | **服务端** `vllm:prefix_cache_hits_total` 增量 > 0 **且** 命中答案与冷路径逐字节相同 | **不要用响应里的 `cached_tokens`（PD 代理不传，恒为 0）**。基线口径已验证可用且正确（144,000 tok 命中、5.7× 加速）；**CED 口径：144K 可用（12/12 正确）、1M 阻断**（命中时连接器的 12-group 形状假设触发 `RuntimeError` 并杀死 D 引擎），见 [`CED-PD-CACHE-HIT-PLAN-20260925.md`](CED-PD-CACHE-HIT-PLAN-20260925.md) §4/§6 与 [`../evidence/ced_prefix_hit_20260926/CED_SIDE_RESULT.md`](../evidence/ced_prefix_hit_20260926/CED_SIDE_RESULT.md) |

...
| 6 | 缓存命中 | **服务端** `vllm:prefix_cache_hits_total` 增量 > 0 **且** 命中答案与冷路径逐字节相同 | **不要用响应里的 `cached_tokens`（PD 代理不传，恒为 0）**。基线口径已验证可用且正确（144,000 tok 命中、5.7× 加速）；**CED 口径也已验证（实验臂 `PREFIX=1` + `V41_CED_ALLOW_PREFIX=1`）**：144K 与 1M 的常规命中、整池命中、交错命中全部正确，命中答案与冷路径逐字节相同；1M 命中 105.7 s → 6.0 s（≈18×）、902909 用例 88.5 s → 5.2 s（≈17×）。期间修掉两个独立的 stock 断言崩溃（P 的 `num_new_tokens > 0`、D 的 `RequestStatus.is_finished`），见 [`CED-PD-CACHE-HIT-PLAN-20260925.md`](CED-PD-CACHE-HIT-PLAN-20260925.md) §4/§6/§10 与 [`../evidence/ced_prefix_hit_20260926/CED_SIDE_RESULT.md`](../evidence/ced_prefix_hit_20260926/CED_SIDE_RESULT.md) |
...
## 7. 目标里「缓存命中」这一项：**已验证，且 2026-09-27 已转为交付默认**

> ⚠️ 本节原文（2026-09-25）写的是"在启动层就被禁止、所以跑不了"。
> 2026-09-26 实测并修掉全部阻断；**2026-09-27 起 `PREFIX` 默认 = 1**，
> 本节据此修正。与上面验收矩阵第 6 行保持一致。
...
**CED 口径的缓存命名：已验证，且 2026-09-27 起就是交付默认（`PREFIX=1`）。**
...
**转正已于 2026-09-27 完成**，实际做法（与原先设想的三步略有出入，以实做为准）：

① `serve_a3_ced_pd.sh` 的默认值改为交付口径：D 侧 `SPEC=1 DRAFT_GRAPH=1`、
   两侧 `PREFIX=1`，并让 decode 默认走**图模式**（`CED_EXPERIMENTAL_GRAPH=1` +
   `V41_CED_GRAPH_PROMPT_TAIL_EAGER=1`）—— 否则裸跑 `decode` 仍会被图模式门拒掉；
② 引擎侧 `patches/files/model.py` 的同一道门默认改为 1（两处必须一起改，
   否则默认启动会在模型构造期崩）；
③ 新增 `tools/selftest_ced_defaults.sh`（11 项，含非法组合负控），
   并入 `tools/selfcheck_pkg.sh` —— 防"默认被某个分支覆盖回旧值"与"门被顺手拆掉"。

> ⚠️ **尚未**用 `PREFIX=1` 重跑那份 21 项完整验收矩阵（已跑的是
> 144K/1M 探针 + 流式 + 并发 4 路 + 两图 + 用户链路，见 §13）。
> 这一格按"已验证但矩阵未补全"记录，不要当成 21/21。

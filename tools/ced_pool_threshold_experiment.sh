...
#   D num_blocks = 30082（P 为 30079，D 只大 3）→ 每 4 个请求失败一次
#   D num_blocks = 30200                        → 每 4 个请求失败一次
#   D num_blocks = 19494（远小于 P）             → 12/12 全过
# 因此候选判别量是「D 的池 > P 的池」，而不是池的绝对大小。
#
# 本脚本依次跑两个配置（都 8 个请求）：
#   A) D 池 ≈ 30000（**小于** P 的 30079）→ 若假说成立，应全过
#   B) D 池 ≈ 30100（**大于** P 的 30079）→ 若假说成立，应出现每 4 个失败
#
...
run_arm below 16230902245   # ≈30000 块（< P 的 30079，D 比 P 小）
run_arm above 16312056756   # ≈30150 块（> P 的 30079，D 比 P 大）
say "全部完成"

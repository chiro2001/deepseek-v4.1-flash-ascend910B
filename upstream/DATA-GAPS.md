...
## 0. 一句话结论（2026-09-21 15:2x 更新）

* **已补 17 项**（G1–G6、G12–G17），**在补 1 项收尾**（G12，测试与锁都已结束，只剩写报告），
  **本机还能做但未排 5 项**（G18–G22，=「待采集」，见 §1.5），
...
### 1.5 ★ "待采集"——本机（A3 空闲 die）还能做、但还没排期的

| # | 缺口 | 为什么值得补 | 难度 |
|---|---|---|---|
| **G18** ★ | **图内（ACLGraph）的 ceiling 扫描** | `logs/41` 的 ceiling 曲线是 **eager** 口径，而**生产帧是图内** —— `V41_ENGRAM_GATE_MAX_TOKENS` 本来就是为 capture 设的常数。子代理自己把这条列为"本任务最大空白"；图内 host 下发被摊掉，曲线形状可能与 eager 不同 | 中：单卡 ACLGraph；脚本骨架已有（`pr/rope_edge_cases.py` 的 graph phase + `agents/T2_ceilings/bench/bench_engram_gate_head2head_sweep.py`） |
| **G19** | engram gate 的**端到端（整 step）放大** | `logs/41` §8 明确标注：函数级 × 层数只是【推断】，**没有实测**。RFC [90] 的收益最终要在 step 上看 | 高：要 8 卡服务 ⇒ 实际落到 G9 |
| **G20** | 8K **小表的时间**（现在只有 kernel 计数） | `logs/41` §6.1 只量了 kernel 数；小表在 wall clock 上的收益没测（生产表 1M 才是目标口径，所以这是补充） | 低：一次 `rope_edge_cases.py --table-size 8192` |
| **G21** | 注册耗时的**双峰/散布成因**（0.0014–0.470 ms/MiB） | `logs/38` §3.3 把它列为未解；`logs/29` 已知第 1 与第 3 分片差 65× 是 page cache 冷热，但完整分布没量 | 低：单卡重复注册同一批文件（`pr/probe_real_engram_table.py` 已能直接跑） |
| **G22** | n > 4096 / 多于 40 层、**跨 CANN·驱动版本稳定性** | `logs/35` §5 挂着的两条。前者低价值（生产上限就是 4096）；后者**本机做不了**（只有一台机器 / 一个 CANN 版本）⇒ 实际属于 §5 | 低（n>4096）/ 不可能（跨版本） |

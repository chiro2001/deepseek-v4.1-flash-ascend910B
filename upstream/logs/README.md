...
| **40** | 2026-09-21 | [`40`](40-20260921-real-table-concurrency.md) | ★★ **真实 206 GiB 表 + 3 die 并发**（补掉 [47] 最硬的一条）—— 3 die 并发满表注册 **240.9/241.3/238.9 s**，**24 次注册全 `ret=0`，无 207001/507011**（#16828 的两个核心判据）；单 die **122.2 s / 0.580 ms/MiB**（复现 `logs/29`）；上游 V2 调用 **83.9 s / 0.398 ms/MiB**。**真表行宽 256 B（不是合成表的 20480 B）⇒ 均匀随机 gather 只有 7.55 GB/s**（合成表 96），但 **热行 skew 在真表上反而有 2.6–4.3× 收益**（与合成表结论相反）；注册**不扰动**其它 die 稳态带宽（≤0.8%）。**另更正 `logs/29` 的"65× 是缓存冷热"**：两片 mincore 都 1.00，根因是**页范围的固有属性**，页缓存最多解释 7–22× |
| **41** | 2026-09-21 | [`41`](41-20260921-engram-gate-ceiling-sweep.md) | ★ **engram gate 的 padding 天花板扫描 + RoPE 两个尾巴** —— 固定 `CHUNK=512` 扫 `MAX_TOKENS` 256→8192：**`t ≈ 0.1 + 0.69×(MAX/512) ms`，CHUNK 以上无拐点**；生产约束（BAT=2048）下 2048 已最优，小 batch 图最优是 512（差 3.4–3.6×，只能靠 per-capture-size 分桶拿掉）；**`MAX=256` 会被静默抬到 4096**（坑）。RoPE：8K 小表 **12.00 → 2.00 kernel**；int32 两格回退**同号复现 4 次**并定位到 host 侧"每方向多 build 一次 index" |
| **42** | 2026-09-21 | [`42`](42-20260921-rope-index-hoist.md) | ★ **把 int32 那两格回退消除掉**（改代码，不是只测）—— `_rope_gather_rows()` 改成接收**已经展平好的 index**，每次调用只 build 一次；改后 **32/32 + 5/5 全过**，两格 **+20.7/+28.9 → −17.9/−17.7 µs**，int32 kernel 数 **4.00 → 3.00**，**27 个计时格全为负**。PR 分支 amend 成 **`ed5b928c`** 并 force-push；组合分支重建为 **`4abfa85e`**；组合校验 `ALL CHECKS PASSED`；单测 **13 passed**（用独立驱动器绕开容器里损坏的 conftest） |
| **43** | 2026-09-21 | [`43`](43-20260921-gate-ceiling-in-graph.md) | ★★ **ceiling 扫描搬进生产帧（ACLGraph capture + replay）+ 8K 小表时间 + n>4096 探测** —— 曲线**仍是直线**：图内斜率 **0.639–0.691 ms/512 行**（eager 0.648–0.710），截距从 0.12 掉到 0.01（n=1）、`MAX=512` vs `2048` 的**绝对差几乎不变（−1.92…−2.10 ms vs eager −2.02…−2.18）**、比值略增（3.6–3.9× vs 3.4–3.8×）⇒ **推荐值不变**。**但"只比上游慢 1.19–1.38×"是 eager 假象**：上游进图后 n=1 从 0.546 掉到 **0.099 ms**（5.5×），我们只快 1–11% ⇒ 图内最优 ceiling 是 **1.41× (512) ~ 6.84× (1)** 上游、shipped 2048 是 **5.1× ~ 26.1×**。126/126 图内格 `torch.equal` + 活性检查通过；L=40（生产 40 层）与 L=16 同值。8K 小表：eager **213.7 → 83.9 µs**、图内 **148.7 → 18.7 µs（7.9×）**；n=8192 图内 12.77 vs 上游 13.26 ms |
| **44** | 2026-09-21 | [`44`](44-20260921-single-session-ablation.md) | ★★ **单会话消融（RFC [97] 的 A1 harness，一直在标 PARTIAL 的那条）** + **两个意外收获**：① **`CPU_BIND=1` 在本机起不了服** —— 8 个 rank 里 2 个卡死在 `migratepages`（NPU12/13 → NUMA node 6，该节点只剩 22 MB 而每 rank 要迁 ~90 GB），**不是慢、是不收敛**（3 分钟三点零进展），进程 `kill -9` 无效、容器删了还在宿主上烧核 ⇒ 已写成 issue 草稿 ④；② ★ **接受长度在两套配置间差 2.7× 且衰减形状不同**（A0 全关 = 1.73、逐位置 {0.50,0.15,0.05,0.02,0.01}；A1 shipped = 4.76、**几乎平坦的 {0.83,0.76,0.72,0.72,0.72}**）—— 各臂内部 6–8 次完全一致 ⇒ **不是噪声，是数值差异**（待二分）。③ 顺带回答 RFC [75] 的 "compile-cache behaviour"：static kernel **每臂全量重编 431 个**、`static_kernel_cache/` 无可复用产物、`compile_outputs/` 3568 个临时目录从未复用、`[SKCACHE-GC]` 因软链从不触发 |

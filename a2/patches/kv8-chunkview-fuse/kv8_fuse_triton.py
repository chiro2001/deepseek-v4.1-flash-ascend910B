...
def _kv8_swa_table_kernel(
    ...
    """Logical block -> scratch page; [FUSE_TUNE] one program per (request, column block).

    ★ 相对 `8057b3eb` 的**唯一**改动（就地把"一个 program 串行 64 次"换成列并行栅格）：
      栅格 `(num_reqs,)` → `(num_reqs, cdiv(width, BLOCK_W))`、`BLOCK_W` 128 → 2048，
      循环体被拆成 `cb = tl.program_id(1)`。`first` / `bpr` / `base` / `val` 的公式**一字未动**，
      写的是同一批值（`out/f5_table_equiv.json`：3 个块宽 × 54 例 `torch.equal` 全 True）。
    实测（c1，7938 页 / 宽 8192 / PP=3 / B=1）：本 kernel 13.1702 → 1.3881 µs/层，
    整条 `fused_ori_plane` 19.6859 → 6.8788 µs/层（`out/f3_kernelcost.json`）。
    通用性：`mask=col < width` 保证任意 `width` 都安全（block 尾部自动掩码）。
    """
    req = tl.program_id(0)
    cb = tl.program_id(1)
    length = tl.load(lens_ptr + req)
...
# [FUSE_TUNE] 表构 kernel 的列块宽（`_kv8_swa_table_kernel` 的第二个栅格维 = cdiv(width, 它)）。
# 原样 8057b3eb 是 `BLOCK_W=128` + **一个 program 循环 64 次**；这里把它摊到
# `cdiv(8192, 2048) = 4` 个 program，实测 13.17 → 1.39 µs/层（生产几何，c1）。
_TBL_BW = 2048
...
def fused_ori_plane(
    ...
    _trace(f"swa table grid=({num_reqs},{(width + _TBL_BW - 1) // _TBL_BW}) width={width} "
           f"pp={pages_per_req} use_qlen={use_qlen}")
    _kv8_swa_table_kernel[(num_reqs, (width + _TBL_BW - 1) // _TBL_BW)](
        table, lens, qsl, width, int(window),
        BS=block_size, PP=pages_per_req, LBS=int(block_size).bit_length() - 1,
        BLOCK_W=_TBL_BW, USE_QLEN=use_qlen, num_warps=1,
    )

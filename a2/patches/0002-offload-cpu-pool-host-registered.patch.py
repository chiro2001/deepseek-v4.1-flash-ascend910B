...
# ---------------------------------------------------------------- [P1_pinned] β
# CPU 池的后端可切换。默认 `registered` = 普通 host 内存 + `aclrtHostRegister(MAPPED)`，
# 绕开驱动的 pinned 池（`aclrtMallocHost`，在 A3 八卡臂上撞过 207001；A2 `host_mem_pool=0`）。
# 依据：`a2/logs/014-20260922-pinned-pool-solution.md`。
_HOST_MEM_MODE = os.environ.get("NPU_OFFLOAD_HOST_MEM", "registered").strip().lower()
_REGISTERED_HOST_BUFFERS: list[tuple[int, int]] = []


def _allocate_npu_offload_cpu_tensor(
    num_blocks: int,
    page_bytes: int,
) -> torch.Tensor:
    """Allocate one worker-side CPU KV block tensor with the configured backend."""
    nbytes = num_blocks * page_bytes
    gib = nbytes / float(1 << 30)
    if _HOST_MEM_MODE == "pinned":  # 旧行为：驱动 pinned 池
        logger.info("[P1_pinned] CPU pool %d x %d (%.2f GiB): pinned", num_blocks, page_bytes, gib)
        return torch.zeros(
            (num_blocks, page_bytes), dtype=torch.int8, device="cpu", pin_memory=True
        )

    tensor = torch.zeros((num_blocks, page_bytes), dtype=torch.int8, device="cpu", pin_memory=False)
    if _HOST_MEM_MODE == "pageable":  # 纯审计用：完全不注册
        logger.info("[P1_pinned] CPU pool %d x %d (%.2f GiB): pageable", num_blocks, page_bytes, gib)
        return tensor
    ...
    try:
        ...
        try:
            acl.init()
        except Exception:  # noqa: BLE001 - 已经 init 过
            pass
        try:
            acl.rt.set_device(int(torch.npu.current_device()))
        except Exception:  # noqa: BLE001
            pass
        devptr, ret = acl.rt.host_register(tensor.data_ptr(), nbytes, 0)  # ACL_HOST_REGISTER_MAPPED
    except Exception as exc:  # noqa: BLE001 - 注册失败就回落，绝不因此起不来
        logger.warning("[P1_pinned] aclrtHostRegister raised: %s", exc)

    if ret == 0:
        _REGISTERED_HOST_BUFFERS.append((tensor.data_ptr(), nbytes))
        logger.info(
            "[P1_pinned] CPU pool %d x %d (%.2f GiB): registered dev=%#x ret=0",
            num_blocks, page_bytes, gib, devptr,
        )
        return tensor
    ...
    return torch.zeros((num_blocks, page_bytes), dtype=torch.int8, device="cpu", pin_memory=True)
...
class NPUOffloadingWorker(CPUOffloadingWorker):
    ...
    def __init__(
        ...
        for kv_cache_tensor in kv_caches.tensors:
            ...
            cpu_tensor = _allocate_npu_offload_cpu_tensor(
                num_cpu_blocks,
                cpu_page_size_bytes,
            )
            logger.debug(

...
def build_offloading_config(
    ...
    # [L3_8card][per-group bpc] dict 形态 => "unit 池"模式（见 a2/logs/021 第 2/3 节）。
    #   池的"一格" = 1 个 GPU block（worker_kv_bytes_per_block x num_copies），
    #   每组的 chunk 占 bpc_g 个 unit（DSV4.1：full=8、SWA=1）。
    #   解析结果写进 extra_config["blocks_per_chunk_by_group"]：调度侧与 worker 侧
    #   各自调用本函数、只依赖 kv_cache_config => 两侧逐字一致（unit/row 映射的前提）。
    if isinstance(blocks_per_chunk_config, dict):
        from vllm.v1.kv_offload.cpu.pgp_manager import (
            BPC_BY_GROUP_KEY,
            resolve_per_group_bpc,
        )

        _scalar_bpc, _bpc_by_group = resolve_per_group_bpc(
            blocks_per_chunk_config, kv_cache_config
        )
        assert _bpc_by_group is not None
        extra_config[BPC_BY_GROUP_KEY] = {
            str(k): int(v) for k, v in _bpc_by_group.items()
        }
        extra_config["blocks_per_chunk"] = int(_scalar_bpc or 1)
        blocks_per_chunk_config = int(_scalar_bpc or 1)
        print(
            "[L3_8card] unit 模式：cache.blocks_per_chunk=%d per-group=%s"
            % (blocks_per_chunk_config, dict(sorted(_bpc_by_group.items()))),
            flush=True,
        )

    # [K_l1_8card][L1] 按组配额（见 a2/logs/042；算法逐字来自 030 的 p2_hooks）：
    #   * 挂载版没有 sitecustomize ⇒ 030 的钩子① 落在这里：把每组的"权重/分量"写进 extra_config；
    #   * 只依赖 kv_cache_config + extra_config ⇒ 调度侧/worker 侧各自算出的结果逐字一致
    #     （这是 unit→row 映射能对齐的前提）；
    #   * P2_POOL_PATCH=0 时**只打日志、不改行为**（= 只读结构臂，本任务第一步）。
    if extra_config.get("blocks_per_chunk_by_group"):
        try:
            from vllm.v1.kv_offload.cpu import p2_pool as _p2

            _bpc_map_l1 = {
                int(k): int(v)
                for k, v in extra_config["blocks_per_chunk_by_group"].items()
            }
            _weights_l1 = _p2.compute_weights(kv_cache_config, groups, _bpc_map_l1)
            _comp_l1, _why_l1 = _p2.compute_components(kv_cache_config)
            _enabled_l1 = _p2.patch_enabled()
            if _enabled_l1:
                extra_config[_p2.WEIGHTS_KEY] = {
                    str(k): int(v) for k, v in _weights_l1.items()
                }
                extra_config[_p2.COMP_KEY] = {
                    str(k): int(v) for k, v in _comp_l1.items()
                }
            _p2.log_weights(_weights_l1, _comp_l1, _bpc_map_l1, _enabled_l1, _why_l1)
        except Exception:  # noqa: BLE001 - 注入失败不能挡住起服，但要响亮
            import traceback

            print(
                "[K_l1_8card][L1] 权重计算失败（不注入，回退 L5 行为）:\n"

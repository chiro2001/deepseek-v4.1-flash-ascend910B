...
| **0003** | **KV8 读侧 rebuild 融合（Triton-Ascend）** | 把「索引算术 → gather → INT8 反量化 → BF16 scratch」压进 **2 个 kernel**（原 ~50 个设备算子）⇒ 整层 **+345.7 → +38.2 µs/层** | `logs/026` |

> ⚠️ **0003 的状态与 0001/0002 不同**：
> * **0001/0002 是 8 卡真权重上验证过的**（`logs/016`）；**0003 只在单卡 c1 上验证**（`logs/026`），
>   而且**它服务于 KV8**（一个"可选增强"），不参与线 1 的卸载上线；
> * `logs/028`（KV8_gate）**独立做出了同一件事**，但产物是**实验脚本**（`p*.py`）而不是可直接挂的补丁
>   —— 两份实现可以互相印证（`+38.2` vs `+45.57 µs/层`），**但没有合并成一份**；
> * **state ring 缩到 FP16**（让容量 ×1.135 → **×1.912**）的改动面是
>   `core/deepseek_v41.py:96-98` + `compressor_triton.py:652` 两处常量/断言（`logs/028` §P1），
>   **尚未打包成补丁**。

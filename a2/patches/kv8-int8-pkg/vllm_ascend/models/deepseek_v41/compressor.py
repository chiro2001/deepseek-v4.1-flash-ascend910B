...
class DeepseekV41CompressorStateCache(DeepseekV41CacheLayer):
    ...
    def __init__(self, vllm_config, prefix, spec):
        if spec.dtype not in RING_STATE_DTYPES or spec.compress_ratio != 1 or spec.block_size != STATE_RING_ROWS:
            raise ValueError("V4.1 compressor state requires a 32-row FP32/FP16 ring")

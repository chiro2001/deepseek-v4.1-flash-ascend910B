...
# [R_ringshrink] The compressor state ring stores the RAW [kv|score] projections
# (not an accumulator) and the pooled output is forced to BF16 anyway, so FP16
# halves the page with strictly less rounding than the output already eats.
# The ring is a *state*: its dtype is a capacity lever, gated by VLLM_V41_RING_FP16.
RING_STATE_DTYPES = (torch.float32, torch.float16)


def ring_state_dtype():
    return torch.float16 if os.environ.get("VLLM_V41_RING_FP16", "0") == "1" else torch.float32


KV8_SCALE_DIM = 4  # one FP16 scale per 128 latent dims (head_size 512 -> 4)


def kv8_long_kv_enabled() -> bool:
    """KV8: store the shared long-KV plane as INT8 + per-128-dim FP16 scales."""
    return os.environ.get("VLLM_V41_KV8", "0").strip().lower() not in ("", "0", "false", "no")


def long_kv_plane_kwargs() -> dict:
    """Spec kwargs for the shared long-KV plane, honouring the KV8 switch."""
    if kv8_long_kv_enabled():
        return {
            "dtype": torch.int8,
            "scale_dim": KV8_SCALE_DIM,
            "scale_dtype": torch.float16,
        }
    return {"dtype": torch.bfloat16}


def kv8_swa_enabled() -> bool:
    """KV8-SWA: store each SWA window plane as INT8 + per-128-dim FP16 scales.

    Without this the hybrid slot page is pinned by the SWA alias
    (``window 128 x head_dim 512 x 2 B = 131072 B``), so quantising the long-KV
    plane alone cannot move ``pool_bytes_per_block``.
    """
    return os.environ.get("VLLM_V41_KV8_SWA", os.environ.get("VLLM_V41_KV8", "0")).strip().lower() not in (
        "",
        "0",
        "false",
        "no",
...
def swa_plane_kwargs() -> dict:
    """Spec kwargs for the SWA window plane, honouring the KV8-SWA switch."""
    if kv8_swa_enabled():
        return {
            "dtype": torch.int8,
            "scale_dim": KV8_SCALE_DIM,
...
class DeepseekV41FullSpec(AscendMLAAttentionSpec):
    def __post_init__(self):
        if self.dtype == torch.int8 and self.scale_dim != KV8_SCALE_DIM:
            raise ValueError("V4.1 INT8 long KV requires one scale per 128 latent dims")
...
class DeepseekV41SWASpec(AscendSlidingWindowMLASpec):
    def __post_init__(self):
        if self.dtype == torch.int8 and self.scale_dim != KV8_SCALE_DIM:
            raise ValueError("V4.1 INT8 SWA cache requires one scale per 128 latent dims")
        if self.dtype != torch.int8 and self.scale_dim:
            raise ValueError("V4.1 SWA scales are only meaningful for an INT8 window plane")
...
class DeepseekV41CompressorStateSpec(AscendCircularBufferSpec):
    ...
    def __post_init__(self):
        if self.dtype not in RING_STATE_DTYPES or self.block_size != STATE_RING_ROWS or self.compress_ratio != 1:
            raise ValueError("Aurora state requires a 32-row FP32/FP16 uncompressed ring")
...
def plan_cache_slots(specs):
    ...
    for slot_idx, kv_name in enumerate(full):
        ...
        # [R8-INT8] 槽位页必须装得下**每一个**叠放在它上面的平面，包括 DSpark draft。
        #   int8 SWA + ring FP16 让 Σstate / Σswa 同时变小 ⇒ 旧式 `max(...)` 会让页
        #   缩到 draft 的 BF16 窗口面以下 ⇒ plan_cache_slots 直接 raise（8 卡真权重实测）。
        #   ★ 没有 draft 组（tiny / 非 DSpark 模型）时本行与旧行为**逐字等价**。
        _draft_size = 0
        if slot_idx < len(draft):
            _draft_size = sum(_cache_plane_sizes(specs[draft[slot_idx]]))
        _alias_max = max(
            (sum(_cache_plane_sizes(specs[n])) for n in aliases), default=0
        )
        capacity = max(kv_bytes + index_bytes, _alias_max, _draft_size)
        if os.environ.get("R8_SLOT_TRACE", "0") == "1":
            print(
                "[R8-SLOTS] slot=%d kv=%d index=%d aliases_max=%d draft=%d capacity=%d "
                "legacy_capacity=%d %s"
                % (
                    slot_idx,
                    kv_bytes,
                    ...
                    _draft_size,
                    capacity,
                    max(kv_bytes + index_bytes, _alias_max),
                    "[draft-aware]"
                    if _draft_size > max(kv_bytes + index_bytes, _alias_max)
                    else "",
                ),
                flush=True,
            )
...
def reshape_cache(raw: torch.Tensor, spec, *, num_blocks, offset, block_stride):
    ...
    if isinstance(spec, DeepseekV41CompressorStateSpec) and (
        sum(plane_sizes) > block_stride or block_stride % spec.dtype.itemsize
    ):
        # [R_ringshrink] Was an equality check ("must fill its slot"). A shrunk
        # ring is smaller than the slot it aliases, and the slot stride then is
        # NOT 32*2*head_size -- the compressor kernels take the page stride from
        # the view (CACHE_PAGE) instead of assuming it.
        raise ValueError("Aurora circular state must fit its slot on a dtype-aligned stride")

...
# [R_ringshrink] the ring page dtype is a capacity lever (see make_ring_fp16_patch.py)
_RING_STATE_DTYPES = (torch.float32, torch.float16)
...
def _pooled_blocked(
    ...
    for chunk0 in range(0, RATIO_PAD, CHUNK_ROWS):
        ...
        if HAS_RES:
            ...
            score_cache = tl.load(
                cache_ptr + (cache_row * CACHE_PAGE + slot[:, None] * 2 * HEAD_DIM) + HEAD_DIM + offs_h[None, :],
    ...
    for chunk0 in range(0, RATIO_PAD, CHUNK_ROWS):
        ...
        if HAS_RES:
            slot = token_pos % CACHE_SIZE
            kv_cache = tl.load(
                cache_ptr + (cache_row * CACHE_PAGE + slot[:, None] * 2 * HEAD_DIM) + offs_h[None, :],
                mask=(row_valid & (seg_off < 0))[:, None],
                other=0.0,
            ).to(tl.float32)
...
def _pool_kernel(
    ...
    CACHE_PAGE: tl.constexpr = 0,
):
    # [R_ringshrink] CACHE_PAGE = state_cache.stride(0) (elements). For a
    # contiguous FP32 ring this equals CACHE_SIZE * 2 * HEAD_DIM, i.e. the
    # change is a no-op; a shrunk ring aliases a larger slot and needs the real
    # page stride.
    pid = tl.program_id(0)
...
def _cache_update_kernel(
    ...
    # [R_ringshrink] CACHE_PAGE = state_cache.stride(0) (elements); see _pool_kernel.
    pid = tl.program_id(0)
    offs_h = tl.arange(0, HEAD_DIM)
    for task_id in range(pid, total_write_slots, tl.num_programs(0)):
        ...
        if valid:
            ...
            tl.store(
                cache_ptr + (cache_row * CACHE_PAGE + slot * 2 * HEAD_DIM) + offs_h,
                kv_vals.to(cache_ptr.dtype.element_ty),
            )
            tl.store(
                cache_ptr + (cache_row * CACHE_PAGE + slot * 2 * HEAD_DIM) + HEAD_DIM + offs_h,
                score_vals.to(cache_ptr.dtype.element_ty),
            )
...
def compressor_from_projected(kv, scores, state_cache, metadata, out, *, max_query_len, num_cores):
    ...
    """
    if kv.dtype != torch.float32 or scores.dtype != torch.float32:
        raise ValueError("Aurora projections must be FP32")
    if state_cache.dtype not in _RING_STATE_DTYPES:
        raise ValueError("Aurora ring state must be FP32 or FP16")
    ...
    if state_cache.stride(2) != 1 or state_cache.stride(1) != 2 * width:
        raise ValueError("Ring state rows must be 2*width wide and contiguous")
    if state_cache.stride(0) < 32 * 2 * width:
        raise ValueError("Ring state page is smaller than 32 rows")
    ...
    # [R_ringshrink] state_cache may be page-strided (ring page < slot page); its
    # layout is validated above instead of via ``is_contiguous``.
    tensors = (kv, scores, metadata, out)
    if not all(t.is_contiguous() for t in tensors):
        raise ValueError("Compressor requires contiguous projections and controls")
    ...
    _pool_kernel[(min(num_cores, batches * groups),)](
        ...
        CACHE_PAGE=state_cache.stride(0),
    )

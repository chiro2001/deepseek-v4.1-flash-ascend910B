...
MOUNT_BLOCK = Anchor(
    ...
    [
        ...
        '  # ★★★ 2026-09-23 10:1x **两条路线必须二选一**（实测事故：A2 起服报',
        '  #     `TypeError: int() argument must be a string … not dict`，来自 `offloading/config.py:78`）。',
        '  #   路线甲（L1_POOL_PATCH=1，**A3 8 卡臂一直用的、有实测**）：**整份替换** 6 个文件',
        '  #     —— `offloading/config.py`（含 dict 解析）+ `cpu/{spec.py,pgp_manager.py,p2_pool.py}`',
        '  #     + native/{npu.py,p2_worker.py}。★ 它同时给出 L1（池按需行数）与 per-group bpc。',
        '  #   路线乙（旧）：挂 `pgp_manager.py` + `pgp_hooks.py`（后者是**运行期 monkeypatch**）。',
        '  #     ★ **它从来没生效过**：`pgp_hooks` 需要有人 import 才会打补丁，而全仓无人 import ⇒',
        '  #     `build_offloading_config` 保持镜像内原样 ⇒ 遇到 dict 就 int(dict) 崩。',
        '  #   ⇒ 两条都挂会**重复挂同一个 `vllm/v1/kv_offload/cpu/pgp_manager.py`** ⇒ docker 直接拒。',
        '  if [ "${L1_POOL_PATCH:-0}" = "1" ]; then',
        '    _L1D="${L1_POOL_DIR:-}"',
        '    [ -n "$_L1D" ] && [ -d "$_L1D" ] || die "L1_POOL_PATCH=1 但 L1_POOL_DIR 不存在：${_L1D:-<空>}"',
        '    for _f in offloading_config.py cpu_spec.py pgp_manager.py p2_pool.py npu.py p2_worker.py; do',
        '      [ -f "$_L1D/$_f" ] || die "L1_POOL_PATCH=1 但缺 $_L1D/$_f"',
        '    done',
        '    _V=/vllm-workspace/vllm/vllm',
        '    _A=/vllm-workspace/vllm-ascend/vllm_ascend/distributed/kv_transfer/kv_pool/kv_offload/native',
        '    MOUNTS+=(-v "$_L1D/offloading_config.py:$_V/distributed/kv_transfer/kv_connector/v1/offloading/config.py:ro")',
        '    MOUNTS+=(-v "$_L1D/cpu_spec.py:$_V/v1/kv_offload/cpu/spec.py:ro")',
        '    MOUNTS+=(-v "$_L1D/pgp_manager.py:$_V/v1/kv_offload/cpu/pgp_manager.py:ro")',
        '    MOUNTS+=(-v "$_L1D/p2_pool.py:$_V/v1/kv_offload/cpu/p2_pool.py:ro")',
        '    MOUNTS+=(-v "$_L1D/npu.py:$_A/npu.py:ro")',
        '    MOUNTS+=(-v "$_L1D/p2_worker.py:$_A/p2_worker.py:ro")',
        '    echo "[serve_a2] [L1-POOL] 已挂 6 文件（per-group bpc 的 config.py/spec.py + 按需行数 p2_*）<- $_L1D"',
        '  else',
        ...
        '    if [ -f "$_PGPH" ]; then',
        '      echo "[serve_a2] WARNING: 只挂了 pgp_hooks.py（monkeypatch）但**没有任何人 import 它** ⇒"',
        '      echo "[serve_a2]           per-group dict 不会生效 ⇒ dict 形式的 blocks_per_chunk 会在"',
        '      echo "[serve_a2]           offloading/config.py 里 int(dict) 崩。请改用 L1_POOL_PATCH=1。"',
        '    fi',
        '  fi',
        'fi',
        ...
        '  # ★★★ 重复挂载的处理（2026-09-23 12:5x 重写；A3 真机踩到）：',
        '  #   生产块在 PATCH_MODE=mount 下**也会挂** `models/deepseek_v41/model.py`，而 int8 块',
        '  #   还要挂同一路径 ⇒ docker 直接报 "Duplicate mount point"（A3 上就是这么失败的）。',
        '  #   ★ 先前的守门有两个问题：① 模式写成 `*/.../model.py`，**匹配不到带 `:rw`/`:ro` 后缀的**',
        '  #     真实挂载串（所以它从来没拦住过）；② 语义是 `die` —— 可 A3 走 mount 模式时**必然**重复，',
        '  #     `die` 等于"int8 在 A3 上永远起不来"。',
        '  #   ⇒ 现在：**int8 版覆盖生产版**（前者是后者的超集，本仓可复算：',
        '  #     `diff patches/files/model.py a2/patches/kv8-int8-pkg/.../model.py` 只有 15 行差异，',
        '  #     且全部是 int8 的 plane kwargs）——把生产块那条**摘掉**，再挂 int8 那条。',
        '  # ★★ 关键细节：MOUNTS 里**每个挂载占两个元素** —— `-v` 与 `路径:目标:mode`',
        '  #   （见上方 `MOUNTS+=(-v "$F/xxx.py:/…:rw")`）。所以"摘掉一条挂载"必须',
        '  #   **成对摘掉**；只删路径会留下孤立的 `-v` ⇒ docker 会把后面的挂载串当成 `-v`',
        '  #   的值、把更后面的东西顶到镜像位置 ⇒ `docker: invalid reference format.`',
        '  #   （2026-09-23 12:5x 真机踩到：第一版就是只删了路径这一半。）',
        '  _kept=()',
        '  _dropped=0',
        '  _n=${#MOUNTS[@]}',
        '  _i=0',
        ...
        '    _val="${MOUNTS[$((_i+1))]:-}"',
        '    case "$_val" in',
        '      */models/deepseek_v41/model.py:rw|*/models/deepseek_v41/model.py:ro)',
        '        _dropped=$((_dropped+1))',
        '        _i=$((_i+2))',
        '        continue',
        '      ;;',
        ...
        '  if [ "$_dropped" != "0" ]; then',
        '    MOUNTS=("${_kept[@]}")',
        '    echo "[serve_a2] [A2-INT8] model.py：摘掉生产块那条挂载（$_dropped 条），改由 int8 版覆盖（int8 版是生产版超集）"',
        '  fi',
        ...
        '            attention/kv8_prefill_triton.py attention/kv8_fuse_triton.py; do',
        ...
        '  echo "[serve_a2] [A2-INT8] 已挂 8 个整文件件（7 个来自 $_I8 + dsa_v41.py）"',
        'fi',
    ],
    "② MOUNTS 卸载/int8 块",
)

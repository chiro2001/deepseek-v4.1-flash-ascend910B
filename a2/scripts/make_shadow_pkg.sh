...
MOUNT_BLOCK = Anchor(
    ...
    [
        ...
        '  # ★★★ 防"重复挂载"（2026-09-22 16:3x 补）：生产块在 PATCH_MODE=mount 下**也会挂**',
        '  #   `models/deepseek_v41/model.py` ⇒ 两个 -v 指向同一容器路径 ⇒ docker 直接报',
        '  #   "Duplicate mount point"（R 在 logs/048 踩过）。这里提前 die，把失败变得响亮、可读。',
        '  for _m in "${MOUNTS[@]+${MOUNTS[@]}}"; do',
        '    case "$_m" in',
        '      */models/deepseek_v41/model.py)',
        '        die "[A2-INT8] model.py 会被**挂两次**（生产块 + int8 块都挂同一路径）。"',
        '      ;;',
        '    esac',
        '  done',

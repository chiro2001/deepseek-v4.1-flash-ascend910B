...
MOUNT_BLOCK = Anchor(
    ...
    [
        ...
        'fi',
        'if [ "${A2_KV8_GRAPHSAFE:-0}" = "1" ]; then',
        '  _DSA="${A2_KV8_DSA:-$PKG/a2/patches/kv8-graphsafe/dsa_v41.py}"',
        '  [ -f "$_DSA" ] || die "A2_KV8_GRAPHSAFE=1 但缺 $_DSA"',
        '  MOUNTS+=(-v "$_DSA:/vllm-workspace/vllm-ascend/vllm_ascend/attention/dsa_v41.py:ro")',
        '  echo "[serve_a2] [A2-OFFLOAD] dsa_v41.py(graphsafe) <- $_DSA"',
        'fi',
    ],
    "② MOUNTS 卸载/int8 块",
)

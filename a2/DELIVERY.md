...
> ## ⚠️⚠️ **2026-09-22 14:2x 新增：一条加强判据出现反例（正在定性）**
>
...
>
> **两种解释，判决实验在做**：
> - **(a) 真缺陷**：int8 命中取回与全量重算不等价；
> - **(b) 量化地板（合法）**：热路径用「池里那行的 int8 反量化值」更新 state、
>   冷路径用「刚算出的 BF16 投影」⇒ **取值来源不同**；
>   `015`/`034` 实测 int8 的 `rel_L2=5.43e-3 / cos=0.9999857` 就是量化地板。
>
> ★★ **判决判据（关键）**：**「冷参考可复现」不能证明「差异是缺陷」** —— 量化本身是确定性的，
> 合法的量化翻转**也必然**系统性可复现。⇒ 决定性实验是 **档 B（BF16 无损池）的 hot vs cold**：
> 若逐字相同 ⇒ 差异只来自 int8 量化 ⇒ (b)，**不是缺陷，而是「判据选错了」**；若不同 ⇒ (a)，**拦发布**。
> ⇒ 详见 `docs/INT8-CASE.md` §4.1。
>
> ★ **与三判据的关系**：目标要求的 **`BlockStored>0` / `CPU→GPU>0` / `hits>0` + replay≪fill** 不受影响（**12.5×**）；
> **「逐字相同」是我们自己加的加强判据** ⇒ 它可以降级，**但对外口径必须跟着降**：
> 定性之前**不要再写「取回后输出与全量重算逐字一致」**。

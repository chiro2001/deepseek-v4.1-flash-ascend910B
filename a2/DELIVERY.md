...
★★ **最锋利的一条判据**：`n-e2-D-hot` 的 mismatch 集合为
`[0,2,3,4,5,6,7,8,10,11,12,13,14,15]`，**恰好多出 prompt 1 与 9** ——
而这两个正是 16 个里**唯一 `ring post nan=0`、`latent nan=0`** 的两个
⇒ **「ring 里有没有 NaN」与「输出对不对」16/16 一一对应**。

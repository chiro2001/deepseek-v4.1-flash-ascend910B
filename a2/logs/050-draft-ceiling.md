...
|---|---|---:|---:|---:|---|
| **1** | ★ **②c draft block 64（保 BF16、保投机）** | **777,318** | **×1.8177** | ×1.6009 | 2 处 + **DRAM 池该组 unit 2→3（Σ权重 20→21 = +5.0%；按 `042 §3` 折算总需求 ≈ +4.8%）**；无精度风险；**判据含 SpecDecoding 四项不降** |
...
## 7.7 ★★★ ②c 的端到端判决（2026-09-22 15:1x–16:0x，8 卡真权重，四臂）

| 臂 | 配置 | Σslot_pages | `GPU KV cache size` | **SpecDecoding（cumulative）** | 判决 |
|---|---|---:|---:|---|---|
| `t-dc2-a-C` | 档 C（long-KV BF16）draft **128** | 540,928 | **427,643** ✅ 与预测逐字同 | **2.42 / 28.4%** | 基线 |
| `t-dc2-c-C3` | 档 C，draft **64**（②c）**但 SWA int8 未生效** | 540,928（**未变**） | **406,425（−5%）** | interval#1 1.01 | ⛔ 该臂**几何不合格**（见 §7.7④） |
| ★ `t-dc2-c-C5` | **档 C（SWA int8 真生效）+ draft64** | **369,280** | ★ **595,404 = 预测逐字** | 只起服、未压测 | ✅ **正收益 ×1.392** |
| `t-dc2-b-D2` | 档 D（long-KV int8）draft **128** | 476,416 | **485,610** ✅ | **3.12 / 42.4%** | 基线 |
| ★ `t-dc2-d-D2` | **档 D，draft 64（②c）** | **282,880** | ★★ **777,318 = 预测逐字** | ★ **1.004 / 0.1%** | ⛔ **容量对、投机塌** |

### ★ ① 容量：**预测被逐字验证**（8 卡 `[R8-SLOTS]` 实测，非算术）
```
档 D + ②c 的 EngineCore trace（core 的 [R8-SLOTS]，无条件打印）：
  slot0: kv=33,280 index=8,320 aliases_max=66,560 draft=65,536 capacity=66,560 legacy_capacity=66,560
  slot1: 同  slot2: 同
  slot3: kv=66,560 index=16,640 aliases_max=66,560 draft=0 capacity=83,200
⇒ Σ = 3×66,560 + 83,200 = 282,880  ✅ 与 §1.2 的【算术】逐字相同
⇒ draft 65,536 < SWA 别名 66,560 ⇒ **binding 从 draft 交回给 SWA**（这正是 ②c 生效的标志）
⇒ 容量 777,318 = ×1.8177 vs 档 B，×1.6009 vs 档 D —— 与 §1.6 零参数模型**逐字相同**（第 8 个预测点）
```

### ⛔ ② **但投机解码塌了** —— 这是 ②c 的否决点
| | 档 C 基线 | 档 D 基线 | ★ 档 D + ②c |
|---|---|---|---|
| MeanAccLen（cumulative） | 2.42 | 3.12 | **1.004** |
| AvgDraftAcc | 28.4% | 42.4% | **0.1%** |
| num_drafts / accepted | 214 / 304 | 166 / 352 | **505 / 2** |

⇒ **接受率从 42.4% 掉到 0.1%**（掉 400×）⇒ **直接违反用户"必须保留投机解码"的硬约束** ⇒
**②c 在当前代码下不可交付**（容量收益 ×1.6009 是真的，但代价是投机解码实效）。

### ★ ③ 机制（【实测·静态】，有来源与先例）
**draft 的页比 slot 的页小 ⇒ draft 视图非连续 ⇒ 读侧按页步长寻址缺失。**
```
位探针（scripts/stride_probe.py，CPU、不占 die，真 spec + 真 reshape_cache）：
  draft=128（现状）：page_stride = 131,072 = 页面内容 131,072 ⇒ 视图 **连续** ✅
  draft= 64（②c）：page_stride =  66,560 > 页面内容  65,536 ⇒ **每页漏 1,024 B** ⇒ 视图 **非连续** ⛔
```
★ **同款先例**：`034` 在 state ring 上踩过**一模一样**的坑 ——
*"ring 减半后页步长 66,560 B 而一页 32 行只占 65,536 B ⇒ **kernel 寻址必须改成按页步长**，否则**静默读错页**"*，
当时的修法是在 kernel 调用点传 **`CACHE_PAGE=state_cache.stride(0)`**（`compressor_triton.py`，8 处接线）。
⇒ **②c 要把 draft 的读路径也接上同样的页步长**（draft 走 `npu_sparse_flash_mla` 的 SWA 分支，需找到它的
`ori_kv` 步长通道），**否则每读一页都错 1,024 B** —— 这与"接受率掉到 0.1%"的现象自洽（draft 的窗口内容被读错）。
**【推断】**（因果未做对照实验：正确的判据是"把页步长接上后接受率回升"）。

### ★ ④ 406,425 的矛盾 —— **已解决**：不是算术错，是那条臂的 **SWA int8 从未生效**
`t-dc2-c-C3`（档 C + draft64）曾实测 **406,425**，而算术预测 **595,404**。**实测 406,425 反解要求 Σ=540,928**（未变）。
⇒ 用**带无条件 trace** 的几何格（`t-dc2-c-C4`，只起服 5 分钟）直接读到了原因：
```
[R8-SLOTS] slot0/1/2: kv=65,536 index=8,320 aliases_max=█131,072█ draft=65,536 capacity=131,072
[R8-SLOTS] slot3:     kv=131,072 index=16,640 aliases_max=131,072 draft=0 capacity=147,712
⇒ Σ = 3×131,072 + 147,712 = 540,928  ⇒ 容量 406,425 ✅ 与实测逐字
```
★ **`aliases_max = 131,072` 就是 BF16 的 SWA 页**（int8 应是 66,560）⇒ **那一条臂的 SWA 量化没生效**。
根因（**同 §7.6 的"静默降级"家族，第三次**）：
```
`swa_plane_kwargs()`（SWA int8 的**唯一**产地）的调用点在
    models/deepseek_v41/model.py:483  AscendDeepseekV41SWACache.get_kv_cache_spec
而我的 runner 当时只在 `KV8LONG=1`（档 D）时才挂这个文件
  ⇒ 档 C 臂里 SWA 平面保持 **BF16(131,072)** ⇒ 档 C 与档 B 的**页几何完全相同**
  ⇒ draft 65,536 仍然远小于 131,072 ⇒ **②c 只让 BPR 涨(2471→2600)、没让 Σ 降** ⇒ −5%
```
**⇒ 修正后重跑（挂上 model.py、SWA int8 真生效）**：`t-dc2-c-C5`（档 C + draft64，只起服）
```
...
⇒ Σ = 3×73,856 + 147,712 = █369,280█    （binding = **long-KV+index**，不是 aliases、也不是 draft）
⇒ GPU KV cache size = █595,404 tokens█   ← ★ 与 §1.6 的算术预测【逐字相同】
```
**⇒ 结论**：① 我的算术是对的；② **档 C + ②c = ×1.392（595,404 / 427,643）是正收益**（不是 −5%）；
③ 那格 −5% 的教训比数字更值钱：**"缺一个挂载文件"会静默把几何降一级** —— 这在本任务里已经出现**三次**
（档 D 缺 model.py ⇒ 降成档 C；档 C 缺 model.py ⇒ 降成档 B；flag 走宿主路径 ⇒ ②c 静默变 draft=128）。
④ ★ **但 ②c 在档 C 上同样有页缝隙问题**：slot 页步长 **73,856** vs draft 页内容 **65,536** ⇒ **缝隙 8,320 B**
（比档 D 的 1,024 B 更大）⇒ §7.7② 的"接受率塌"机制**对档 C 只会更严重**（未做接受率实测 ⇒ 标【推断】）。

### ★ ⑤ 端到端链上踩到的**三个工程坑**（都留档，后人省时间）
1. **档位被静默降级**：自建 runner 没挂 `models/deepseek_v41/model.py`（`serve_a2.sh:1001-1003` 只在
   `R8_MERGED_MODEL` 指向存在的文件时才挂）⇒ `long_kv_plane_kwargs()` 从未被调用 ⇒ **跑成档 C**（427,643 而非 485,610）。
   ⇒ 已加两道防线：`KV8LONG=1` 找不到合并版就 `exit 64` + 起服前打印"本臂 = 档 C/档 D"。
2. **`VLLM_V41_KV8_GRAPH_SAFE` 必须为 1**（图模式 + 档 D）：否则首个真实请求就
   `507057 SUSPECT REMOTE ERROR` 崩引擎（`t-dc2-b-D-legacy` = `049 §5.5.3` 那条反例臂的**第三次独立复现**；
   而 `t-dc2-b-D2` 置 1 后同几何**跑通**）。⇒ runner 已默认为 1，并自动拷 S 的 `dsa_v41.py`（`94aeebb7`）。
3. **两条"宿主变量进不了容器"的假设**（都靠**把开关写进文件本身**解决）：
   * `draft_block_64.flag`：我建在宿主 `$T/`，但 `r8-*` 容器由 `serve_a2.sh` 起、**没有 /work 挂载**
     ⇒ 容器里那个路径**不存在** ⇒ 静默回落到 draft=128（`t-dc2-c-C2` 因此作废）；
   * `R8_SLOT_TRACE`：**从来没进过 inner.sh 的 export 列表**（`patch_serve_a2_int8.sh:144-152` 只转发 `VLLM_V41_*`）
     ⇒ 三次 `grep R8-SLOTS` 全 0 命中。
   ⇒ **修法**：生成器新增 `--variant b64`（**无条件** `block_size=64` 写进被挂载的 core）与 `--always-trace`
     （把 trace 的 env 门拆掉）。**教训：白名单 env + 路径假设 = 静默失效；把开关钉进被挂载的文件才可靠。**
4. **runner 的假阴性门**：`R_8card_int8/run_arm_r8.sh` 的自检 grep `$PKG/results/$RID/serve.log`，
  而 `serve_a2.sh` 这次把引擎日志写到 `$OUT/serve.log` ⇒ 128 行 `P1_pinned` 被数成 **0** ⇒ 假阴性 FATAL、容器被删、压测没跑
  （`AGENTS §5b` 第 1/2 条的变体）。⇒ 本任务改用自有 runner，按**正确路径**收证据，并加**早停闸**：
  `DRAFT64=1` 时若 group 清单里 draft 组不是 64 ⇒ 立刻停机（**不浪费 25 min 跑完才发现**）。

### ★★ ⑥ **②c 被否掉之后，路线排序要翻转：②a 更干净**（【实测·静态】，位探针）

同一把位探针跑三种 draft 形态（`--draft-int8 1` / `--draft-block 64` / `--draft-block 128`，档 D 几何）；
容量用 §1.6 的零参数模型（**必须同时算 Σ 与 BPR**）：

```
max_in_flight = 2×8192 = 16,384；window = 128 ⇒ n = min(127+16384, 133120) = 16,511
p_swa   = 16511//128 + 1 = 130
p_draft = 16511//block + 1  ⇒  block 128 → 130 ；block 64 → 259
BPR     = 1040 + 1 + 10×130 + p_draft        （全走档 D 的 long-KV/state/SWA 几何）
tokens  = (4 GiB // Σ − 1) // BPR × 133,120
```

| draft 形态 | 页内容 | slot 页步长 | **缝隙** | Σslot_pages | **p_draft** | **BPR** | blocks | **预测容量** |
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| BF16 block 128（现状） | 131,072 | 131,072 | **0** ✅ | 476,416 | 130 | 2471 | 9,014 | 485,610 **【实测】** |
| **②c BF16 block 64** | 65,536 | 66,560 | **1,024 B** ⛔ | 282,880 | **259** | **2600** | 15,182 | **777,318【实测，但接受率塌】** |
| ★ **②a INT8 block 128** | **66,560** | **66,560** | ★ **0** ✅ | **282,880** | ★ **130** | ★ **2471** | 15,182 | ★ **817,898（×1.6843）【推断】** |

★★ **三条结论（含一处自我更正）**：
1. ⛔ **我上一版写"②a 与 ②c 容量相同（都是 777,318）"是错的 —— 主代理当场更正，我用自己的模型复算确认**：
   **Σ 相同 ≠ 容量相同**。②a 的 draft 块**仍是 128** ⇒ `p_draft = 130`（与现状同）⇒ **BPR 保持 2471**；
   而 ②c 的 draft 块是 64 ⇒ `p_draft = 259` ⇒ **BPR 涨到 2600**（块表/记账的每请求页数翻倍）。
   ⇒ **②a = 817,898，比 ②c 的 777,318 多 40,580 token（+5.2%）**。
   ★ 复算方式是**直接跑 §1.6 的零参数模型**（8 卡口径）：`485,610 → 777,318 → 817,898` 三行，
   其中前两行是**实测逐字命中**、第三行是【推断】（模型在 8 卡上已由 4 个点支持：档 B/C/D 的 draft128 三点 + ②c 的 777,318）。
2. ★★★ **②a 的页恰好是 66,560 = slot 页步长 ⇒ 缝隙 0**，而 **②c 的页 65,536 < 66,560 ⇒ 缝隙 1,024 B**
   ⇒ **②a 没有"非连续/页步长"这个坑**（这正是 ②c 塌掉接受率的最可能机制，§7.7③）。
   ★ 依据算术：`128×(512×1 + 4×2) = 65,536 + 1,024 = 66,560`，与 target SWA int8 的页**逐字相同**
   ⇒ **②a 让 draft 与 target SWA 的页几何完全一致**（`plan_cache_slots` 里"draft 装得下"的检查也天然成立）。
3. ★ **两档 × 三种 draft 形态的完整表**（零参数模型，8 卡口径；【推断】行已标注）：
| 档 | draft 形态 | Σslot_pages | p_draft | BPR | **预测容量** | vs 该档现状 |
|---|---|---:|---:|---:|---:|---:|
| C | 现状 BF16 128 | 540,928 | 130 | 2471 | 427,643 **【实测】** | ×1.0000 |
| C | **②c BF16 64** | 369,280 | 259 | 2600 | **595,404【实测·§7.7④】** | **×1.3923** |
...
★ 读法：**②a 在每一档都比 ②c 多 5.2%**，因为 **②a 的 Σ 与 ②c 相同、但 BPR 不涨**（draft 块仍是 128）。

⇒ **路线排序翻转（幅度比上一版写的更大）**：**②a 在容量上比 ②c 多 5.2%，且没有页缝隙问题**
   （代价：draft 走 int8 读路径 —— `kv8_ori_plane` 同款重建 + 那个 `pages_per_req=2` 硬编码 + 图捕获，见 §3.6）；
   **②c 只有在"先修 draft 读路径的页步长"之后才值得考虑**。
   ★ 与主代理派出的 `DS_draft_graph_int8`（②a 病行探针）方向一致。

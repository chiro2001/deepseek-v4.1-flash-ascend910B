...
| `t-dc2-c-C3` | 档 C，draft **64**（②c） | 540,928（**未变，见下**） | **406,425（−5%）** | interval#1 1.01 | ⛔ **负收益** |
...
### ★ ④ 另一格未解的口径差异（**不许用相邻数字顶替**）
`t-dc2-c-C3`（档 C + draft64）实测 **406,425**，而【算术】预测 **595,404**。
反解：`(4294967296/Σ − 1)/BPR × 133120 = 406,425` 在已知的 `BPR=2600` 下要求 **Σ=540,928**（= 未变），
而按 §1.2 的算术 档 C 里 draft 从 131,072 降到 65,536 会让容量降到 `max(73,856, 66,560, 65,536)=73,856` ⇒ Σ=369,280。
⇒ **两条对不上**，且那一条臂**没有** `[R8-SLOTS]` trace（当时还没加）⇒ **【未确认】**。
★ 需要一格（起服 3 分钟、不必压测）才能定：档 C + b64t（带无条件 trace）⇒ 直接读 `slot0..3` 的五个数。
...
3. ⇒ **路线排序翻转（幅度比上一版写的更大）**：**②a 在容量上比 ②c 多 5.2%，且没有页缝隙问题**

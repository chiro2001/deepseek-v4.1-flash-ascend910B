...
> ⚠️ 为什么做不到 ≈0%：rebuild 的定义就是「把 INT8 物化成 BF16」，**把 SWA 整条抹成 0，
> 光 4 层 cmp 也还要 127 µs = +0.4%**（判据 2×）。要 ≈0% 只能**让 attention 算子直读 int8**
> —— 那是**要新算子**（移植 A5 的 `mixed_quant_sparse_flash_mla`），不在本期范围。

...
### 4.1 ★★★ **2026-09-22 14:5x 已定性：「热 replay ≠ 冷算参考」不是 int8 缺陷，是判据判别力不足**

> ★ **结论先给（主代理从原始 `client.json` 独立核实）**：**档 B（BF16 无损池，四个 int8 开关全 0）的热臂，
> 给出的退化 token 与档 C（int8 池）的热臂【逐字相同】** ⇒ 那 2/16 的差异**来自「热/取回路径」本身，不是 int8**。
> ⇒ ★★ **这不是「发现了缺陷」，而是「这条判据在 `max_tokens=1` 的 8 卡口径下判别力不足」**（理由见下）。
> ⇒ 但不许外推成「int8 已被证明保真」—— **正面判据（KV 级逐字节）仍未跑**。
...
| **(b) 判据判别力不足**（★ **实测支持这一支**） | ① **BF16 无损池也给出同样的退化 token** ⇒ 差异来自热/取回**路径本身**；② `max_tokens=1` ⇒ 只生成 **1 枚 token**，候选是**近平局**（全是空白类 `'\n'`/`' '`/`'_'`）⇒ argmax 极易翻转 | **四臂并排实测**（`logs/062` §1）：档 B hot == 档 C hot（逐字）；fill 轮四臂全同；★ 但 **档 D 在 p9 上不一致**（`' '` vs `'_'`）【未确认】 |

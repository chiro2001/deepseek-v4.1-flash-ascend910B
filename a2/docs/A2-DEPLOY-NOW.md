...
>    ```
>    ★★ **统一假说【待证】**：档 D 在**前置负载之后**，某些 prompt 的 **decode 步 logits 被损坏**；
>    * 请求 logprobs ⇒ 从损坏的 logits 算 logprob ⇒ **NaN / 400**
>    * 不请求      ⇒ 从**同一个**损坏的 logits 采样 ⇒ **token 不同 / 文本变短**
>    ⚠️ **所以"生产不请求 logprobs 就不会有问题"是错的** —— 不请求只是**看不到 400**，
>    仍可能**静默拿到错的 token**。
>    ⇒ ★ **档 D 挡死**；`a2/scripts/serve_a2_offload.sh` 的推荐配置里**不含 `KV8_FULL`/`KV8_PREFILL`**。
>    ⇒ **档 C 不受影响**（三条独立判据：`same=8/8` + `NaN=0` + 本条的 B 档对照）。

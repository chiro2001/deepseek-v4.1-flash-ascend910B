...
# 「shadow 存在性检查 + 拷 4 个补丁 + `export OFFLOAD_*_PATCH=1` + `export P2_*`」**整段吞进 int8 分支**
# ⇒ **不开 int8（档 B）时**：① 4 个卸载补丁**一个都不挂**（服务照常 READY，但**零卸载**，零报错）；
# ② `P2_POOL_PATCH`/`P2_COMP_JSON` 不导出 ⇒ **L1 失效** ⇒ 宿主内存 **≈3.49× 池 → ≈7× 池**（85 GiB 池：≈297 → **≈590 GiB**）。
#
# ★ **上车前必做**：`git pull`（修在 `7c1edf2`）—— 修后：公共 export 移出分支 + 起服前 fail-closed
# 断言两个补丁开关 + `DRY=1` 断言真实 MOUNTS 里必须有那 4 个补丁文件名。详见 [`logs/074`](../logs/074-20260922-launch-path-p0-fixed.md)。
# ★ 本文件 §1/§2 的命令本来就**显式**写了 `OFFLOAD_SCHED_PATCH=1 OFFLOAD_NPU_WORKER_PATCH=1`（照它抄是对的），
# 但它们**没有**写 `P2_POOL_PATCH` ⇒ 旧脚本下 L1 仍不生效（宿主 197 → 392 GiB）。**pull 之前别照抄。**

# ⛔⛔⛔⛔ **窗口前必读（2026-09-22 20:33 新增，**这是第 4 个独立阻塞项，且它直接命中目标配置**）**

> ## `ENGRAM=1` + 卸载：**一旦发生池命中，replay 轮引擎就死（`KeyError: 2486`）**
>
> ### 判决读数（三臂，A3 8 卡真权重）
>
> | 臂 | `ENGRAM` | 池 | **有命中？** | **replay** | `KeyError` |
> |---|---:|---:|---|---:|---:|
> | `p1b` | 0 | 56 GiB | 有 | **failed=0** | 0 |
> | **`p2e`** | **1** | **56 GiB** | **有** | ★ **failed=13**（引擎死） | **32** |
> | ★ **`p2f`** | **1** | **1 MiB** | ★ **无** | ★★ **failed=0** | ★ **0** |
>
> ⇒ ★★★ **`p2e` 与 `p2f` 的唯一变量 = 池大小（⇒ 是否取回）**，其余配置逐字相同
> ⇒ **崩的不是"Engram × 长 prompt"，是「Engram × 卸载取回」。**
>
> ### 根因（精确到一行，见 `logs/073` §10）
>
> ```
> engram_jit_kernel.py:63-65  docstring 原文：
>   err_page >= 0 : 标量回退时读到缺页 → 调用方抛 KeyError
> engram_jit_kernel.py:148-158  「缺页回退：标量走法」
>   page = flat[r * lookback + sh]
>   ★ if page_present[page] == 0:      ← Engram 认为"它要读的那页 KV 不在视野里"
>         err_page = page              ← 2486 就是这个页号
> ```
> 而 `engram_hash.update()` 的 docstring 原文：**"page numbers come from full SWA KV"**。
> ⇒ ★★ **字节确实从 DRAM 取回了（`CPU→GPU>0`），但 Engram 自己的 `page_present` 表没被告知那些页可用**
> ⇒ Engram 读到"缺页" ⇒ `KeyError` 逃逸 ⇒ 引擎死。
>
> ### ⛔ 对窗口的直接含义
>
> ```
> ★ 这不是"边缘工况"：A2 的目标配置就是 ENGRAM=1 + 卸载，
>   而【任何一次取回】都会触发。⇒ 当前配置【不能上】。
> ```
>
> ### 三条可选出路（**都需要用户决策或额外实验**）
>
> | # | 出路 | 代价 / 前置 |
> |---|---|---|
> | **1** | ★ **在 `engram_hash` 的调用方捕获该 `KeyError` 并走 host 路径**（`model.py:1092` 目前不处理它） | 改动小；但要确认 host 路径的数值语义等价 |
> | **2** | ★ **让取回路径也更新 Engram 的 `page_present`**（根因修） | 改动较深，要动卸载/Engram 交界 |
> | **3** | ⚠️ **让 A2 不开 Engram（`ENGRAM=0`）+ 开卸载** | ★ **质量问题**（Engram 是模型的组成部分：2 层 + 206 GiB 表）⇒ **必须用户决策** |
>
> ⇒ ★★ **在出路 1/2 修好之前，A2 窗口只能用"两个都不开"或"二选一"** —— 详见 `logs/073`。
>
> ---
>
...
### A0. ⛔⛔⛔ **先核这一条：「返回的文本正确吗」—— 三判据只证"有没有搬"，不证"搬来的对不对"**

> **2026-09-22 20:0x 用户提出（主代理采纳为窗口的【第 0 道门】）**：
> *"需要测试其能够返回正确的文本。"*
>
> ## 为什么它必须排在 A 节之前
>
> 下面 A 节的五条判据（`BlockStored>0` / `CPU→GPU>0` / `hits>0` / `replay÷fill` / `BlockRemoved==0`）
> **全部是"计数器级"** —— 它们只回答"**有没有搬、搬了多少、快不快**"，
> **一条都不回答"搬来的字节对不对"**。
>
> ★ 四家子代理今晚**独立**收敛到这一点（见 `logs/071` §0）：
> ```
> 8 卡：证据全是计数器（CPU→GPU=21.5 GB / hits=901,120）
> 单卡卸载：sha 只有 2 轮 + 每 prompt 1 token
> 单卡 int8：全部臂在 --load-format dummy 下 ⇒ 被探 KV 平面 n_nonzero=0 ⇒ "clean" 是平凡的
> 源码侧：device-index 只证"注册被接受"
> ```
> ★ **本仓已有先例**：`043` 发现 group 0 **每次只搬 1/8 个 block**（"读到未写过行 5352 → 0"）——
> 那**不是**靠常规判据发现的，是**靠自建探针**。
>
> ## 怎么核（三条，从便宜到贵；★ 窗口里至少做第 1 条）
>
> ### 1. ★★ **语义正确性（固定题，有唯一答案）** —— 最低成本，直接回答用户的问题
>
> ```bash
> # 在 A2 宿主上（服务在跑）
> Q() { curl -s localhost:8077/v1/chat/completions -H 'Content-Type: application/json' \
>   -d "{\"model\":\"deepseek-v4-flash\",\"messages\":[{\"role\":\"user\",\"content\":\"$1\"}],\"max_tokens\":64,\"temperature\":0}" \
>   | python3 -c 'import json,sys; d=json.load(sys.stdin); print(d["choices"][0]["message"]["content"])'; }
>
> Q "计算 17×23 等于多少？只给数字。"        # ⇒ 期望包含 391
> Q "天空为什么是蓝色的？一句话。"            # ⇒ 期望包含 瑞利 / 散射
> Q "《红楼梦》的作者是谁？只给名字。"        # ⇒ 期望包含 曹雪芹
> Q "40×60%÷2 等于多少？只给数字。"           # ⇒ 期望包含 12
> Q "把字符串 Hello, world! 反转输出。"       # ⇒ 期望包含 !dlrow ,olleH
> ```
> **判据**：五条**全部**答对 ⇒ 过。任一答错 ⇒ ★ **停下，先查是不是取回路径**（把 `OFFLOAD_GB` 调到 0 或重启冷服务再问同一题）。
> ★ 这组题来自本仓历史质量判据（`066`：10 条全过），**有历史对照**。
>
> ### 2. ★ **同输入一致性（★ 必须 3 轮）**
>
> ```
> ★ 规则（logs/071 §3 C1）：同输入一致性判据**必须跑 ≥3 轮取"任一不同"** ——
>   2 轮会漏掉 40%（实测：fill!=replay 只抓到 [4,5,7]，而三轮任一不同是 [0,3,4,5,7]）
> ★ 且注意 037 已证：本服务在 temperature=0 下**本就有抖动** ⇒
>   单轮 sha 差异**不能**单独当缺陷；要看"三轮任一不同"的集合，并与对照臂比
> ```
>
> ### 3. ⏳ **取回路径的字节级保真（真正的判据，但目前【未跑】）**
>
> ```
> 同臂内「写侧的 GPU 页」vs「取回后的 GPU 页」逐字节比对
> ⇒ 需重建 transfer_async 的指针表（新探针工程，独占一轮）
> ⇒ 这是 logs/071 §1 的 A1：目前**唯一的判据缺口**
> ```
>
> ### 附：金标准（有历史绝对值，但**不能替代** 1/2/3）
>
> | 项 | 历史值 | 工具 | ⚠️ 局限 |
> |---|---|---|---|
> | Vision 23 例 | **23/23** | `<shadow-pkg>/tools/vision_accuracy_check.py` | 短 prompt ⇒ **很可能不触发取回** |
> | GSM8K-200 | **197–199** | `tests/t_gsm8k.py` | 同上 |
>
> ⇒ ★ **它证明"模型没坏"，但证明不了"取回的数据对"** —— 两条不同的判据。

...
| ⚠️ **越界读风险** | 【实测·机制】`[SG-PPR]` 证明捕获期 `cmp_mcs=6 ⇒ ppr=1`，而 replay 的压缩前缀有几百块；**后果 = ★ 两种都可能**（2026-09-22 更正）：越界读【表】通常只读到同进程垃圾，但那个垃圾"页号"再乘 `cmpKvStride0` 取 KV 时 —— 落已映射 ⇒ **静默算错**、落未映射 ⇒ ★ **实测崩引擎**（`507057 SUSPECT REMOTE ERROR`）。⇒ **必须验 sha**，不许用"反正会崩"自我安慰 |

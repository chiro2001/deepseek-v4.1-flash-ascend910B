...
## ★ 现在到底能不能上（2026-09-22 15:0x 快照）

| 档 | 8 卡真权重 | 图模式 | HBM 容量 | 宿主内存（16×128K） | 结论 |
|---|---|---|---|---|---|
| **档 B** | ✅ | ✅ | 427,643 | **197.21 GiB** | ✅ 可上线 |
| **★ 档 C**（int8 SWA+ring16） | ✅ | ✅（`sg-c-c-graph-b` 全绿，读数与旧版**逐字相同**） | 427,643（×1.0000） | ★ **150.01 GiB**（×1.3146） | ✅ **推荐** |
| **★ 档 D**（+KV8 双平面） | ✅ | ✅（`sg-c-d-graph` 全绿，`replay1 sha == eager` **逐字节**） | ★ **485,610**（×1.1356） | ≈174 GiB【推断】 | ✅ 可上线（★ 一条保留：接受率需另测） |
| **②c**（draft block 64，保 BF16） | ⏳ 端到端在 c0 排队 | ✅ 单 die（b64/b128 捕获 23 s / 9 s，`EE1016=0`） | 预测 **777,318**（×1.8177） | — | ⏳ 在验（Q3 已过：提案序列 4367/4367 逐条相同） |
| ⛔ **②a**（draft **INT8**，block 保持 128） | ⛔ **不可用** | ✅ 单 die（`capture_finished=1`，`EE1016=0`） | 预测 817,898（×1.9126，tiny 实测 39,846 逐字命中模型） | — | ⛔ **Q3 实测失败：引擎在第一个 spec-decode step 就死**（`056`） |

**★ 三条运行判据（DRAM 卸载）在 8 卡上的实测**：`hits=901,120` / `CPU→GPU=21.19 GB` / `replay÷fill=12.50×`。

> ⚠️ **三条挂在台面上的保留意见**（不藏）：
> 1. **档 D 的接受率** —— 现有读数（`max_tokens=1`、`Drafted 15`）**样本太小、不足以判定**；
>    已在 ②c 链里排了**同几何 + `max_tokens≥64`** 的基线来补这一格。★ 判据用「图模式 vs eager 逐字相等」，**不用绝对值**。
> 2. ★★ **原始目标 `×1.84` 未达（现状）** —— 实测档 C **×1.0000** / 档 D **×1.1356**；
>    ★ **能碰到那一档的是「解开 draft 天花板」的两条路**，而其中：
>    · **②a（draft INT8，预测 ×1.9126，超过 ×1.84）已实测失败** —— 引擎在**第一个 spec-decode step** 就崩（`The previous device metadata submission has not been released`）；
>      同包同参数、唯一变量 `DRAFT_INT8=0` 的对照臂**全绿 16/16** ⇒ **②a 特有，不是 harness**；
>    · **②c（draft BF16 + block 64，预测 ×1.8177）仍在验**，Q3 已过（提案序列 4367/4367 逐条相同）。
...
> 加上 `max_tokens=1` 只生成 1 枚 token、候选是近平局 ⇒ **「逐字相同」这条加强判据在该口径下判别力不足**。
> ⚠️ 但**不等于「int8 已被证明保真」** —— 正面判据（KV 级逐字节）**仍未跑**；
> **档 D 在 prompt 9 上留一格【未确认】**。详见 [`logs/062`](logs/062-hot-cold-verdict.md)。

...
| ★★ **②a**（draft **INT8**，block **保持 128**） | ⏳ 未排 | ✅ **单 die 实测**（`capture_finished=1`，`EE1016=0`） | ★ 预测 **817,898**（**×1.9126**，tiny 实测 39,846 逐字命中模型） | — | ⏳ **缺 Q3（接受率）** |
...
>    ★ **能碰到那一档的是「解开 draft 天花板」的两条路**（见上表 ②c / ②a 两行）；
>    其中 **②a 的预测 ×1.9126 已经超过 ×1.84**，但**它的 Q3（接受率）还是空白** ⇒ **不能按它收口**。
>    详见 [`docs/INT8-CASE.md`](docs/INT8-CASE.md) §1 与 [`logs/059`](logs/059-draft-int8-roadmap.md)。

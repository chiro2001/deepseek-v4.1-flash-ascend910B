...
| **②c**（draft block 64） | ⏳ 端到端在 c0 排队 | ✅ 单 die（b64 / b128 捕获 23 s / 9 s，`EE1016=0`） | 预测 **777,318**（×1.8177） | — | ⏳ 在验 |
...
> 2. **原始目标 `×1.84` 未达** —— 实测档 C **×1.0000** / 档 D **×1.1356**；
>    ★ **只有 ②c 能碰到那一档**（预测 ×1.8177）。详见 [`docs/INT8-CASE.md`](docs/INT8-CASE.md) §1。
> 3. **A2 本机的池后端探测仍是唯一阻塞**（§0.1）—— 只能在那台机器上做。

### 当前正在跑的（都不踢锁）
```
c0：sg-c-c-eager（档C eager、补丁关 —— 反例臂）→ c-eager-on → c-cold（冷算参考）
     ②c 端到端两臂（t-dc2-b-D → t-dc2-c-D）排队中
```

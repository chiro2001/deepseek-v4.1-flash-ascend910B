...
| `docs/` | **上线清单**（`A2-GO-LIVE.md`：参数定值、四条判据、并发×上下文边界） |
...
## 5. ★★ 并发 × 上下文的容量边界（**先看这张表再定参数**）

per-group bpc 之后，池子的宿主实占（×6.945）：
...
| | 要覆盖的 | 1.000× 池 |
|---|---|---|
| 最坏 | Σ 各请求长度 | 16 × 128K ⇒ 264 GiB |
| **典型（共享前缀）** | 唯一前缀总量 | ≈1 个前缀 ⇒ **16.5 GiB** |

**⇒ 差 16 倍**，所以"能扛多少并发"要用**真实 trace 的前缀唯一率**算，不能按请求数乘积估。

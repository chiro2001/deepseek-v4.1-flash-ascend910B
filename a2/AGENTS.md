...
16. ★★★ **"同输入应当一致"的判据，必须跑 ≥3 轮并取「任一不同」—— 2 轮会漏掉一半**
   实例（2026-09-22，`c2` 的档 D 判决，主代理独立复算）：
   ```
   同一批 8 条 prompt，同进程、同三档：
     判据 A（弱）：fill != replay     ⇒ [4, 5, 7]          ← 只有 3 条
     ★ 判据 B（强）：fill/replay/replay2 任一不同 ⇒ [0, 3, 4, 5, 7]   ← 完整 5 条
   原因：p0/p3 的损坏**第 3 轮才出现**（48→48→10、152→152→19）
   ```
   ⇒ ★ **"延迟到第 N 轮才显现"正是【进程内累积型】缺陷的典型形态** ——
     用 2 轮判据会把它们读成"正常"，进而得出"两条独立缺陷"这种错误结论。
   ★ **判据**：凡是"同一输入多次应当给同一结果"的验证（回放一致性、缓存命中一致性、
     确定性检查），**至少 3 轮**；且**报告里要写清用的是几轮**。
   ★ 配套：**三档（B/C/D）用同一个判据跑**，才能排除"判据强度不同"这个混淆 ——
     本例中 B/C 在该强判据下**连一条都不同都没有**，所以"档 C 安全"不是侥幸。

17. ★★★ **"生产默认值"是配置的判据 —— 任何交付脚本都必须与它逐项对齐，不一致就要响亮报错**
   本晚共踩 **7 次**，全是同一个形状：**默认值 ≠ 生产，且程序不报错**。
   ```
   ① docker exec 不继承宿主 env（LIGHT/A2PROBE_FLOOR_GIB 被丢）
   ② 累加 pinned 吃满 floor ⇒ 后面的注册段被整段跳过
   ③ T 的 runner 少挂 model.py ⇒ 档 D 静默跑成档 C
   ④ 传内部名 A2_KV8_* 而非用户接口 KV8_* ⇒ 档位静默回退
   ⑤ DSPARK_DISPATCH_QUERY_LEN_FIX 只在注释里、无 -e 透传
   ⑥ APC_ALIGN 未接上（且 048 的负载让它"没被触发"，所以没暴露）
   ⑦ ★ ENGRAM_DEVICE_INDEX 默认 auto vs 生产 0 ⇒ 在 A2 上会把 device-index 打开
      ⇒ 正好走进 A3 那条崩掉的路（EH0012 + 183 GiB 注册）
   ```
   ★ **硬规则**：
   1. 交付脚本里**每一个会影响代码路径的开关**，都要**显式写出默认值**；
   2. 该默认值**必须等于生产**（不是"更安全的猜测"）；
   3. 用户显式改成非生产值时 ⇒ **打印警告 + 给出依据**（不许静默接受）；
   4. ★ 上线判据里要有一条**"确认开关真的生效/关闭"**的判据（如本例的 `grep -c DEVICE-INDEX` = 0）。

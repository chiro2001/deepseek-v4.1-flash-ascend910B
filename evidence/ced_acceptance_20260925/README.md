...
| **multiturn（三轮）** | 见 §2（修正后 3/3 PASS） | 见 §2（修正后 3/3 PASS） |
...
⚠️ 这两组是**全 40 层基线**上的重跑。CED 臂的多轮（同样是 18K / 125K 的缩水版本）
**仍需在 CED 上重跑**才能替换掉旧记录。
...
needle 四针与流式的**正确性**；以及**全 40 层基线**在修正后 144K 与 1M
三轮多轮上的**正确性**（含 999,510–999,562 token 的近满上下文）。

**不能支持**：

* "**CED 臂**的 144K/1M 多轮" —— 基线臂已按真实长度重跑通过（§2），
  CED 臂的旧记录仍是缩水版本，需在 CED 上重跑。
* "缓存命中" —— 假通过（§3）。

...
from vllm_ascend.utils import enable_custom_op  # production registration path
assert enable_custom_op(), "production custom-op registration failed"
...
try:
    ...
    a_finite = bool(np.isfinite(out_a).all())
    a_precheck = {
        'arm': 'A',
        'finite': a_finite,
        'row125_dim1': float(out_a[125, 0, 1]),
        'row126_dim1': float(out_a[126, 0, 1]),
        'row127_dim1': float(out_a[127, 0, 1]),
        'output_sha256': hashlib.sha256(np.ascontiguousarray(out_a).view(np.uint8)).hexdigest(),
    }
    print('ATTENTION_A_PRECHECK ' + json.dumps(a_precheck), flush=True)
    if not a_finite:
        raise RuntimeError('attention arm A produced non-finite output; arm B not run')
    out_b = call_attention(q, ori_b, ori_table_b, seq_ori_b, meta_b, *op_args)
    b_finite = bool(np.isfinite(out_b).all())
    print('ATTENTION_B_PRECHECK ' + json.dumps({
        'arm': 'B', 'finite': b_finite,
        'row125_dim1': float(out_b[125, 0, 1]),
        'row126_dim1': float(out_b[126, 0, 1]),
        'row127_dim1': float(out_b[127, 0, 1]),
        'output_sha256': hashlib.sha256(np.ascontiguousarray(out_b).view(np.uint8)).hexdigest(),
    }), flush=True)
    if not b_finite:
        raise RuntimeError('attention arm B produced non-finite output')
    ...
    def summarize(name, out, ref):
        ...
        return {
            ...
            'row125_dim1': float(out[125, 0, 1]),
            'row126_dim1': float(out[126, 0, 1]),
            'row127_dim0': float(out[127, 0, 0]),
            'row127_dim1': float(out[127, 0, 1]),
            'cpu_row0_dim0': float(ref[0, 0, 0]),
            'cpu_row0_dim1': float(ref[0, 0, 1]),
            'cpu_row125_dim1': float(ref[125, 0, 1]),
            'cpu_row126_dim1': float(ref[126, 0, 1]),
            'cpu_row127_dim0': float(ref[127, 0, 0]),
            'cpu_row127_dim1': float(ref[127, 0, 1]),
            'output_sha256': hashlib.sha256(np.ascontiguousarray(out).view(np.uint8)).hexdigest(),
        }
    ...
    result = {
        ...
        'cmp_upper_bound_and_common_softmax': {
            'valid_cmp_count_rows_125_126_127': [valid_cmp[125], valid_cmp[126], valid_cmp[127]],
            'row125_dim1_A_B': [float(out_a[125, 0, 1]), float(out_b[125, 0, 1])],
            'row126_dim1_A_B': [float(out_a[126, 0, 1]), float(out_b[126, 0, 1])],
            'row127_dim1_A_B': [float(out_a[127, 0, 1]), float(out_b[127, 0, 1])],
            'row126_cpu_A_B': [float(ref_a[126, 0, 1]), float(ref_b[126, 0, 1])],
            'row127_cpu_A_B': [float(ref_a[127, 0, 1]), float(ref_b[127, 0, 1])],
            'max_abs_A_vs_B_all_128_rows': float(np.abs(out_a - out_b).max()),
            'max_abs_A_vs_B_dim1_all_rows': float(np.abs(out_a[:, 0, 1] - out_b[:, 0, 1]).max()),
        },
    ...
    assert abs(result['A']['row125_dim1']) <= 1e-2
    assert abs(result['B']['row125_dim1']) <= 1e-2
    assert abs(result['A']['row126_dim1'] - 100.0 / 255.0) <= 1e-2
    assert abs(result['B']['row126_dim1'] - 100.0 / 254.0) <= 1e-2
    assert abs(result['A']['row127_dim1'] - 100.0 / 255.0) <= 1e-2
    assert abs(result['B']['row127_dim1'] - 100.0 / 255.0) <= 1e-2
